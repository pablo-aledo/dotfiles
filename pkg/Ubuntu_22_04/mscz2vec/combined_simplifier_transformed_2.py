...
║   dificultad pedagogica (--target-grade 1..8, de principiante a virtuoso),  ║
║   reduciendo el numero de notas y la complejidad de la textura mientras se  ║
║   preserva en la medida de lo posible la identidad armonica y melodica de   ║
║   la obra original. Para cada grado pedido genera un fichero .mid nuevo con ║
║   la version simplificada.                                                  ║
║                                                                              ║
║  TECNICAS:                                                                  ║
║   1. Analisis estructural (motor principal): separa la textura en voces de ║
║      melodia y bajo (skyline), detecta la armonia por compas, clasifica    ║
║      cada nota segun su funcion tonal (tono del acorde, nota de paso, nota ║
║      de bordado, apoyatura, suspension, anticipacion, escapada...) y le    ║
║      asigna un peso estructural. A partir de ese peso calcula un "suelo"   ║
║      de notas protegidas mediante reduccion schenkeriana progresiva (nunca ║
║      se eliminan) y aplica despues un algoritmo greedy que va retirando,   ║
║      compas a compas, las notas menos importantes hasta alcanzar el grado  ║
║      de dificultad pedido (medido con una formula de 5 factores: velocidad ║
║      de notas por segundo, extension de la mano, polifonia simultanea,     ║
║      saltos melodicos e independencia entre manos).                        ║
║   2. Rejilla de respaldo (mecanismo de garantia): en piezas muy densas en   ║
║      acordes en bloque, el suelo estructural puede impedir que el greedy   ║
║      llegue al grado pedido. Cuando esto ocurre, se aplica una segunda     ║
║      tecnica independiente: cuantizar los onsets a una rejilla temporal    ║
║      cada vez mas gruesa y limitar la polifonia a un numero maximo de      ║
║      voces por mano (agudo-primero en la mano derecha, grave-primero en la ║
║      izquierda), estirando las notas resultantes en legato. Las notas      ║
║      protegidas por el suelo estructural nunca entran en esta rejilla; la  ║
║      nota que sobrevive en cada casilla se elige segun el peso estructural ║
║      ya calculado en el paso 1. Esta rejilla se aplica de forma progresiva ║
║      (peldaño a peldaño, cada uno partiendo del resultado del anterior)    ║
║      solo hasta que se alcanza el grado pedido, para minimizar cuanto se   ║
║      aleja el resultado de la pieza original.                              ║
║   3. Transformaciones sin eliminacion (nuevo en v2.0): ademas de retirar   ║
║      notas, se aplican tecnicas que SUSTITUYEN, REDISTRIBUYEN o incluso    ║
║      AÑADEN notas -- armonicas (sustituir extensiones por el tono de       ║
║      acorde mas cercano, clarificar el bajo), melodicas (resolver          ║
║      adornos breves, reducir saltos grandes por reubicacion de octava),    ║
║      de registro (cerrar voicings abiertos), de textura (arpegiar         ║
║      bloques de acorde densos, colapsar dobles de octava), ritmicas       ║
║      (cuantizado suave sin recorte de polifonia) y aditivas controladas   ║
║      (rellenar huecos grandes dejados por una reduccion agresiva con una  ║
║      nota de paso/apoyo). Las armonicas/melodicas se aplican como pre-    ║
║      pasada uniforme antes del greedy; las de registro/textura/ritmica    ║
║      forman una escalera intermedia entre el greedy estructural y la      ║
║      rejilla de respaldo; las aditivas son una pasada de acabado final.   ║
║      Desactivable con --no-transforms / --no-additive.                    ║
║                                                                              ║
║  USO:                                                                       ║
║    python combined_simplifier.py obra.mid --target-grade 1 3 5 7           ║
║    python combined_simplifier.py obra.mid --target-grade 2 --outdir out/   ║
║    python combined_simplifier.py obra.mid --target-grade 4 --split 60      ║
...
def reduce_graded(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
    ...
    # pasada previa [T1]/[T2] ARMONICA + MELODICA: sustituciones que no
    # cambian el numero de notas, aplicadas de forma uniforme ANTES del
    # greedy para todos los target_grades (igual que el adelgazado de
    # voicings), de modo que el greedy ya opere sobre una superficie
    # armonica/melodicamente mas simple.
    pre_simplify_stats = {"armonia_sustituida": 0, "bajo_clarificado": 0,
                           "adornos_resueltos": 0, "saltos_reducidos": 0}
    if pre_simplify:
        ...
        pre_simplify_stats["saltos_reducidos"] = reduce_melodic_leaps(records)
...
# ══════════════════════════════════════════════════════════════════════════════
#  [TRANSFORM] TECNICAS DE SIMPLIFICACION QUE NO ELIMINAN NOTAS
#
#  Todo lo anterior (suelo estructural + greedy + rejilla) reduce dificultad
#  eliminando notas. Esta seccion añade seis familias de tecnicas que
#  simplifican SUSTITUYENDO, REDISTRIBUYENDO o incluso AÑADIENDO notas, en
#  vez de solo retirarlas. Se dividen en:
#
#    1. RITMICAS   — cuantizar suavemente, alinear ataques entre manos.
#    2. DE REGISTRO — cerrar voicings abiertos, corregir saltos melodicos
#                     grandes por reubicacion de octava.
#    3. DE TEXTURA  — arpegiar bloques de acorde densos, colapsar
#                     duplicaciones de octava dentro de un mismo bloque.
#    4. ARMONICAS   — sustituir tonos de extension/alterados por el tono de
#                     acorde mas cercano, clarificar el bajo en cada cambio
#                     armonico.
#    5. MELODICAS   — resolver adornos muy breves (bordados/apoyaturas/
#                     escapadas) fundiendolos en la nota vecina, reducir
#                     saltos melodicos grandes por reubicacion de octava.
#    6. ADITIVAS    — rellenar huecos melodicos grandes dejados por una
#                     reduccion agresiva con una nota de paso/apoyo, como
#                     pasada final de acabado (nunca para "ganar grado").
#
#  Todas las funciones de esta seccion operan sobre List[NoteRec] y respetan
#  SIEMPRE floor_protected (nunca tocan pitch/tiempo de una nota del suelo
#  estructural) y nunca tocan la voz mel/bass salvo que se indique
#  explicitamente lo contrario (reduce_melodic_leaps, resolve_ornaments).
# ══════════════════════════════════════════════════════════════════════════════

# ── [T1] ARMONICAS ─────────────────────────────────────────────────────────

def _nearest_chord_tone_pitch(pitch: int, chord: Optional[ChordSpan]) -> int:
    """Devuelve `pitch` desplazado al tono de acorde mas cercano (mismo
    registro aproximado, desplazamiento minimo en semitonos con signo)."""
...
def simplify_harmony(records: List["NoteRec"], spans: List[ChordSpan], tc: "TimeContext",
    ...
    """Sustituye (no elimina) tonos de extension/alterados de voces internas
    por el tono de acorde mas cercano, cuando la nota dura lo suficiente
    como para no ser una figuracion melodica de superficie (esas las trata
    resolve_ornaments/reduce_melodic_leaps, no esta funcion)."""
    count = 0
    for r in records:
...
def clarify_bass(records: List["NoteRec"], spans: List[ChordSpan], tc: "TimeContext") -> int:
    """En cada cambio de armonia, si la nota mas grave de la mano izquierda
    sonando en ese instante no es la fundamental del acorde, la desplaza
    (por semitonos, sin cambiar de octava) a la fundamental — clarifica el
    bajo funcional sin retirar ninguna nota."""
    count = 0
    lh = [r for r in records if r.hand == "lh"]
    for sp in spans:
        if sp.root is None:
        ...
        if bass_r.floor_protected or bass_r.voice_role == "bass":
            continue  # el bajo estructural (skyline grave) no se toca aqui
...
def resolve_ornaments(records: List["NoteRec"], tc: "TimeContext",
    ...
    """Funde adornos muy breves (NT/APP/ESC de duracion tipo mordente) en la
    nota vecina de su propia voz (melodia o bajo), igual que hace el suelo
    estructural con la extension de huecos, pero como sustitucion dirigida
    por duracion en vez de por peso/dificultad."""
...
def reduce_melodic_leaps(records: List["NoteRec"], leap_threshold: int = 9) -> int:
    """Cuando dos notas consecutivas de la misma voz (melodia o bajo, en la
    misma mano) forman un salto mayor que `leap_threshold` semitonos,
    reubica la segunda por octavas (conservando su clase de tono) al valor
    que minimiza el salto, si eso no colisiona con la anterior."""
    count = 0
    by_voice: Dict[Tuple[str, str], List["NoteRec"]] = defaultdict(list)
    for r in records:
        if r.voice_role in ("mel", "bass"):
    ...
    for lst in by_voice.values():
        ...
        for i in range(1, len(lst)):
            ...
            if cur.floor_protected:
                continue
            leap = cur.note.pitch - prev.note.pitch
            if abs(leap) <= leap_threshold:
                continue
            best_pitch, best_leap = cur.note.pitch, abs(leap)
            for oct_shift in (-24, -12, 12, 24):
                cand = cur.note.pitch + oct_shift
                if not (0 <= cand <= 127):
                    continue
                cand_leap = abs(cand - prev.note.pitch)
                if cand_leap < best_leap:
                    best_leap, best_pitch = cand_leap, cand
            if best_pitch != cur.note.pitch and best_leap <= leap_threshold:
                cur.note.pitch = best_pitch
                count += 1
    return count
...
def close_voicings(records: List["NoteRec"], max_span: int = 12) -> int:
    """Cuando un bloque simultaneo (misma mano, mismo onset) tiene una
    extension mayor que `max_span` semitonos, reubica por octavas las voces
    internas (no los extremos agudo/grave, que definen el contorno) para
    comprimir el voicing a una posicion mas cerrada, sin retirar ninguna."""
    count = 0
    ...
    for group in by_group.values():
        if len(group) < 3:
            continue
        group_sorted = sorted(group, key=lambda r: r.note.pitch)
        low, high = group_sorted[0], group_sorted[-1]
        if high.note.pitch - low.note.pitch <= max_span:
            continue
        existing = {r.note.pitch for r in group_sorted}
        for r in group_sorted[1:-1]:
            if r.floor_protected:
                continue
            p = r.note.pitch
            while p - low.note.pitch > max_span:
                p -= 12
            while p < low.note.pitch:
                p += 12
            if p != r.note.pitch and p not in existing:
                existing.discard(r.note.pitch)
                existing.add(p)
                r.note.pitch = p
                count += 1
    return count
...
def collapse_octave_doubles(records: List["NoteRec"]) -> Tuple[List["NoteRec"], int]:
    """Dentro de un mismo bloque simultaneo (misma mano, mismo onset), si dos
    voces internas suenan exactamente a una octava de distancia, retira la
    superior y deja sonar la inferior (colapso de voces dobladas). Nunca
    toca melodia/bajo ni notas protegidas por el suelo."""
    to_remove: Set[int] = set()
    by_group: Dict[Tuple[str, int], List["NoteRec"]] = defaultdict(list)
    for r in records:
        by_group[(r.hand, r.note.start)].append(r)
...
def arpeggiate_dense_chords(records: List["NoteRec"], tc: "TimeContext",
                             min_notes: int = 4, min_duration_beats: float = 0.75) -> int:
    """Convierte bloques de acorde muy densos (>= min_notes voces
    simultaneas, misma mano) en un arpegio ascendente rapido dentro de la
    misma duracion total: cada voz conserva su pitch y su final, pero su
    ataque se reparte en el tiempo. No se retira ninguna nota; baja la
    polifonia simultanea pico a cambio de subir ligeramente notas/seg, asi
    que solo se aplica a bloques con duracion suficiente para no disparar la
    velocidad."""
    count = 0
    by_group: Dict[Tuple[str, int], List["NoteRec"]] = defaultdict(list)
    for r in records:
        by_group[(r.hand, r.note.start)].append(r)
    for group in by_group.values():
        if len(group) < min_notes or any(r.floor_protected for r in group):
            continue
        if any(r.voice_role in ("mel", "bass") for r in group):
            continue  # no desplazar el ataque de la voz principal/bajo
        template = group[0]
        if template.note.duration_beats(tc) < min_duration_beats:
            continue
        sg = sorted(group, key=lambda r: r.note.pitch)
        span_ticks = max(1, template.note.end - template.note.start)
        slice_ticks = max(1, span_ticks // (2 * len(sg)))  # arpegio rapido, no rompe el pulso
        for i, r in enumerate(sg):
            new_start = r.note.start + i * slice_ticks
            r.note.start = new_start
            r.note.end = max(r.note.end, new_start + 1)
            count += 1
    return count
...
def quantize_light(records: List["NoteRec"], grid_ticks: int) -> int:
    """Cuantiza los ataques (no protegidos por el suelo) a `grid_ticks`,
    desplazando el final en la misma medida para conservar la duracion.
    A diferencia de la rejilla de respaldo, esto NO recorta polifonia ni
    fusiona notas: solo alinea el ritmo de superficie (util para reducir
    sincopas superfluas e independencia entre manos)."""
    count = 0
    for r in records:
        if r.floor_protected:
            continue
        old_start = r.note.start
        new_start = int(round(old_start / grid_ticks)) * grid_ticks
        if new_start != old_start:
            shift = new_start - old_start
            r.note.start = max(0, new_start)
            r.note.end = max(r.note.start + 1, r.note.end + shift)
            count += 1
    return count


# ── [T6] ADITIVAS CONTROLADAS ────────────────────────────────────────────────

def add_connective_notes(records: List["NoteRec"], tc: "TimeContext",
                          gap_threshold_beats: float = 1.0, min_leap: int = 5) -> Tuple[List["NoteRec"], int]:
    """Pasada de ACABADO (nunca para 'ganar grado'): cuando una reduccion
    agresiva ha dejado un hueco grande y un salto amplio entre dos notas
    consecutivas de melodia/bajo, inserta UNA nota de paso/apoyo a mitad de
    camino en pitch, corta y marcada floor_protected (no vuelve a competir
    por eliminacion), para regularizar la linea en vez de dejarla vacia."""
    added: List["NoteRec"] = []
    by_voice: Dict[Tuple[str, str], List["NoteRec"]] = defaultdict(list)
    for r in records:
        if r.voice_role in ("mel", "bass"):
    ...
    for (hand, role), lst in by_voice.items():
        ...
        for i in range(len(lst) - 1):
            ...
            added.append(NoteRec(note=new_note, hand=hand, bar=tc.bar(start), voice_role=role,
                                  function="PT", weight=0.2, floor_protected=True,
                                  is_octave_dup=False, candidate=False, orig_id=-1))
    return records + added, len(added)


# ── ESCALERA DE TRANSFORMACIONES (entre el greedy estructural y la rejilla) ─

def apply_transform_ladder_until_target(records: List["NoteRec"], tc: "TimeContext",
                                         spans: List[ChordSpan],
                                         target_grade: int) -> Tuple[List["NoteRec"], int, bool, List[str]]:
    """Aplica, en orden creciente de intrusividad, las tecnicas de sustitucion
    y redistribucion (registro -> melodica -> textura -> ritmica) — cada
    peldaño parte del resultado del anterior — hasta alcanzar target_grade o
    agotar la escalera. A diferencia de la rejilla, NINGUN peldaño de esta
    escalera elimina notas por si solo (salvo collapse_octave_doubles, que
    fusiona dobles de octava exactas, no notas distintas)."""
    cur = list(records)
    steps: List[str] = []
    ...
    grade = _grade(cur)
    if grade <= target_grade:
        return cur, grade, True, steps

    n = close_voicings(cur)
    if n:
        steps.append(f"registro:voicings_cerrados({n})")
    grade = _grade(cur)
    if grade <= target_grade:
        return cur, grade, True, steps

    n = reduce_melodic_leaps(cur)
    if n:
        steps.append(f"melodica:reduccion_saltos({n})")
    grade = _grade(cur)
    if grade <= target_grade:
        return cur, grade, True, steps

    cur, n = collapse_octave_doubles(cur)
    if n:
        steps.append(f"textura:colapso_octavas({n})")
    grade = _grade(cur)
    if grade <= target_grade:
        return cur, grade, True, steps

    n = arpeggiate_dense_chords(cur, tc)
    if n:
        steps.append(f"textura:arpegiado({n})")
    grade = _grade(cur)
    if grade <= target_grade:
        return cur, grade, True, steps

    n = quantize_light(cur, tc.tpb // 4)
    if n:
        steps.append(f"ritmica:cuantizado_ligero({n})")
    grade = _grade(cur)
    return cur, grade, grade <= target_grade, steps
...
def combined_simplify(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
    ...
    for t in sorted(set(target_grades), reverse=True):
        ...
        if achieved <= t:
            ...
            # peldaño intermedio [TRANSFORM]: sustituir/redistribuir/
            # arpegiar/cuantizar antes de recurrir a la rejilla dura.
            transformed, achieved_t, ok_t, steps = apply_transform_ladder_until_target(
                snap, tc, spans, t)
            if ok_t:
                method = "estructural+transformaciones (" + " → ".join(steps) + ")" if steps else "estructural+transformaciones"
                chosen, achieved_final = transformed, achieved_t
            else:
                # la escalera de transformaciones no bastó sola: se recurre a
                # la rejilla, pero partiendo del snapshot ESTRUCTURAL original
                # (no del ya transformado) — la cuantizacion ligera de la
                # escalera desplaza onsets de una forma que no es la que
                # espera el clustering de la rejilla, y arrancar la rejilla
                # desde ahi es menos eficiente (conserva menos notas) que
                # arrancarla directamente desde el suelo estructural.
                ...
                method = base + " (transformaciones intentadas sin exito antes de la rejilla)"
        ...
        # [T6] pasada de acabado ADITIVA: solo si la reduccion fue agresiva
        # (menos de la mitad de las notas originales sobreviven) y solo de
        # cara a regularizar huecos, nunca para perseguir el grado.
        if use_additive and n_original and (len(chosen) / n_original) < 0.5:
            chosen, n_added = add_connective_notes(chosen, tc)
            if n_added:
                method += f" +aditivas({n_added})"
...
def print_combined_report(midi_path: str, out: Dict):
    ...
    print(f"\n{'═'*78}\n  COMBINED SIMPLIFIER v2.0 (autocontenido) — {midi_path}\n{'═'*78}")
    ...
    if pss and any(pss.values()):
        print(f"  pre-pasada armonica/melodica (uniforme para todos los grados): "
              ...
              f"{pss['saltos_reducidos']} saltos melodicos reducidos")
...
def main():
    ...
    ap.add_argument("--no-transforms", action="store_true",
                     help="desactiva las tecnicas de sustitucion/redistribucion "
                          "(armonicas, melodicas, de registro, de textura, ritmicas) "
                          "y vuelve al comportamiento v1.1 (solo suelo+greedy+rejilla)")
    ap.add_argument("--no-additive", action="store_true",
                     help="desactiva la pasada final de notas de paso/apoyo "
                          "añadidas tras reducciones agresivas")
    args = ap.parse_args()
    ...
    out = combined_simplify(args.midi, args.target_grade, floor_level=args.floor_level, split=args.split,
                             use_transforms=not args.no_transforms, use_additive=not args.no_additive)
