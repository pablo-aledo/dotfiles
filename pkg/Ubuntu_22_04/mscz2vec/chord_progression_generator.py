...
def resolve_numeral(numeral: str, tonic_pc: int, complexity: float = 0.5) -> tuple[str, list[int]]:
    ...
    # Solo añadir M7 a acordes mayores diatónicos explícitamente mayores (no alterados)
    # para evitar extensiones fuera de escala en modo menor
    diatonic_major_numerals = {'I', 'IV', 'V', 'II', 'III', 'VI', 'VII'}
    if complexity > 0.6 and quality == '' and numeral in diatonic_major_numerals:
        quality = 'M7'
...
def score_pattern(pattern: list, tension_curve: list, n_bars: int,
    ...
    for numeral, dur in (pattern * (int(n_bars / bars_per_pattern) + 2)):
        ...
        fn = HARMONIC_FUNCTION.get('minor', {}).get(numeral,
             HARMONIC_FUNCTION.get('major', {}).get(numeral, None))
