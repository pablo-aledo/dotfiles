...
# ──────────────────────────────────────────────────────────────────────────────
#  NOVEDADES v4.0 [ARREGLO] (derivadas de comparar la salida v3 contra una
#  simplificacion manual de referencia; todas activas por defecto):
#   · detect_and_fix_meter    — corrige metro declarado erroneo (4/4 vs vals 3/4)
#   · choose_transposition    — transporta a la tonalidad con menos teclas negras
#   · protect_melody          — la linea melodica es intocable para greedy/rejilla
#   · _snap_composite         — cuantizacion binaria+tresillos (no destruye tresillos)
#   · retexture_accompaniment — regenera la MI como patron regular desde la armonia
#   · thin_rh_to_melody       — la MD lleva la melodia (+ refuerzo puntual)
#   · normalize_melody_durations — limpia duraciones de MIDI 'de interpretacion'
#   · formula de dificultad de 7 factores (añade teclas negras e irregularidad
#     ritmica: sin ellas el optimizador no podia 'ver' estas ganancias)
#  Flags para desactivar: --no-meter-detect --no-transpose --no-protect-melody
#  --no-retexture --retexture-max-grade N
# ──────────────────────────────────────────────────────────────────────────────
...
def _black_key_load(notes: List[Note]) -> float:
    """Fraccion (ponderada por duracion) de notas en tecla negra. Un 50%+
    de teclas negras (p.ej. Fa menor) es objetivamente mas dificil para un
    estudiante que la misma pieza en La menor; la formula original de 5
    factores era ciega a esto."""
    tot = sum(max(1, n.end - n.start) for n in notes)
    if tot <= 0:
        return 0.0
    blk = sum(max(1, n.end - n.start) for n in notes if n.pitch % 12 in _BLACK_PCS)
    return blk / tot


def _irregularity(notes: List[Note], min_iois: int = 6) -> float:
    """Irregularidad ritmica de una mano: 1 - fraccion de IOIs (intervalos
    entre onsets consecutivos) cubierta por los DOS valores de IOI mas
    frecuentes. Un acompañamiento en patron regular (vals, Alberti) puntua
    ~0.1; una textura de duraciones erraticas puntua alto. Es el factor que
    hace visible al optimizador la ganancia real de regularizar la textura
    (la version manual de referencia era mas facil precisamente por esto,
    no por tener menos notas)."""
    onsets = sorted({n.start for n in notes})
    if len(onsets) < min_iois + 1:
        return 0.0
    iois = defaultdict(int)
    for a, b in zip(onsets[:-1], onsets[1:]):
        iois[b - a] += 1
    tot = sum(iois.values())
    top2 = sum(sorted(iois.values(), reverse=True)[:2])
    return 1.0 - top2 / tot


# Pesos de la formula de dificultad de 7 factores (v4.0). Los 5 originales
# se reescalan para hacer sitio a 'negras' e 'irregularidad'.
GRADE_WEIGHTS = {"velocidad": 0.25, "extension": 0.15, "polifonia": 0.17,
                 "saltos": 0.11, "independencia": 0.12,
                 "negras": 0.08, "irregularidad": 0.12}
...
def bar_factors(rh_bar: List[Note], lh_bar: List[Note],
    ...
    negras = float(np.clip(_black_key_load(rh_bar + lh_bar) / 0.5, 0, 1))
    irr = 0.5 * _irregularity(rh_ctx) + 0.5 * _irregularity(lh_ctx)
    irregularidad = float(np.clip(irr / 0.6, 0, 1))

    factors = {"velocidad": speed, "extension": extension, "polifonia": poly,
               "saltos": leaps_f, "independencia": independence,
               "negras": negras, "irregularidad": irregularidad}
    diff = sum(factors[k] * GRADE_WEIGHTS[k] for k in factors)
...
def whole_piece_grade(rh: List[Note], lh: List[Note], tc: TimeContext) -> Tuple[int, float]:
    ...
    negras = float(np.clip(_black_key_load(all_notes) / 0.5, 0, 1))
    irr = 0.5 * _irregularity(rh) + 0.5 * _irregularity(lh)
    irregularidad = float(np.clip(irr / 0.6, 0, 1))
...
def reduce_graded(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
                   ...
                   climax_bars: Optional[Set[int]] = None,
                   auto_meter: bool = True,
                   auto_transpose: bool = True,
                   protect_melody: bool = True) -> dict:
    ...
    all_notes = sorted((n for trk in mid.tracks for n in extract_notes(trk)),
                       key=lambda n: (n.start, n.pitch))
    if not all_notes:
        raise ValueError("el MIDI no contiene notas")

    # [ARREGLO] correccion de metro: el motor entero razona 'por compas',
    # asi que hay que hacerlo ANTES de crear TimeContext y de todo analisis.
    meter_fix = detect_and_fix_meter(mid, all_notes) if auto_meter else None

    # [ARREGLO] transposicion a tonalidad comoda: antes del analisis
    # armonico, para que acordes, suelo y clasificacion vivan ya en la
    # tonalidad final (y el MIDI exportado salga transportado).
    bk_before = _black_key_load(all_notes)
    transpose_shift = choose_transposition(all_notes) if auto_transpose else 0
    if transpose_shift:
        _transpose_events(mid, transpose_shift)
        all_notes = sorted((n for trk in mid.tracks for n in extract_notes(trk)),
                           key=lambda n: (n.start, n.pitch))
    bk_after = _black_key_load(all_notes)
    ...
    # [ARREGLO] la melodia es intocable: se marca DESPUES de la pre-pasada
    # (para que resolve_ornaments aun pueda fundir adornos, la unica
    # simplificacion melodica legitima) y ANTES del greedy/rejilla, que asi
    # jamas podran eliminarla ni recuantizarla — la referencia manual
    # conservaba el 76% de la melodia en posicion exacta; la rejilla del
    # motor v3 a grado 4 solo conservaba el 24%.
    n_melody_protected = 0
    if protect_melody:
        for r in records:
            if r.voice_role == "mel":
                r.floor_protected = True
                r.candidate = False
                n_melody_protected += 1
    ...
    return {
        ...
        "meter_fix": meter_fix, "transpose_shift": transpose_shift,
        "black_keys_before": bk_before, "black_keys_after": bk_after,
        "n_melody_protected": n_melody_protected,
    }
...
# ══════════════════════════════════════════════════════════════════════════════
#  [ARREGLO] TECNICAS DE ARREGLISTA (v4.0) — lo que un simplificador manual
#  hace y la resta estructural no: corregir el metro declarado, transportar a
#  una tonalidad comoda, tratar la melodia como intocable, regenerar el
#  acompañamiento como patron idiomatico regular y normalizar duraciones.
#  Derivadas de comparar la salida del motor contra una simplificacion manual
#  de referencia (wchb_0 -> wchb_2): la version manual conservaba el 88% de
#  las notas y aun asi era mucho mas facil — la facilidad venia de estas
#  operaciones, no de borrar notas.
# ══════════════════════════════════════════════════════════════════════════════

def detect_and_fix_meter(mid: MidiData, notes: List[Note],
                          candidates: Tuple[int, ...] = (2, 3, 4, 6),
                          min_ratio: float = 1.12) -> Dict:
    """Estima el metro real a partir del perfil de acentos de los onsets
    (duracion + gravedad de las notas que atacan en cada tick) y, si
    contradice con evidencia suficiente al compas declarado en el MIDI,
    corrige mid.timesig_map in place (incluida la FASE de la barra: donde
    cae el tiempo 1). Todo el motor es 'por compas' (pesos metricos, greedy,
    calificacion, rejilla), asi que operar sobre barras falsas — p.ej. un
    vals en 3/4 exportado con el 4/4 por defecto del secuenciador — degrada
    todas las decisiones. Devuelve un dict informativo."""
    tpb = mid.tpb
    declared = mid.timesig_map[0][1] if mid.timesig_map else 4
    acc: Dict[int, float] = defaultdict(float)
    for n in notes:
        w = 1.0 + min(4.0, (n.end - n.start) / tpb) + max(0.0, (60 - n.pitch) / 24.0)
        acc[n.start] += w
    total = sum(acc.values())
    info = {"declared": declared, "detected": declared, "phase": 0,
            "ratio": 1.0, "changed": False}
    if total <= 0:
        return info

    tol = tpb // 4

    def down_score(num: int, phase: int) -> float:
        barlen = num * tpb
        s = 0.0
        for t, v in acc.items():
            off = (t - phase) % barlen
            if off <= tol or off >= barlen - tol:
                s += v
        return s * num / total  # 1.0 = acentos uniformes (sin evidencia)

    best_by_num: Dict[int, Tuple[float, int]] = {}
    for num in candidates:
        best_by_num[num] = max((down_score(num, ph * tpb), ph * tpb)
                                for ph in range(num))
    # preferencia por el compas MAS CORTO que explique los acentos casi
    # igual de bien: un vals con agrupacion hipermetrica fuerte-debil
    # puntua algo mas alto en 6/4 que en 3/4, pero el compas pedagogica y
    # notacionalmente correcto es 3/4 (el 6/4 solo se elige si su evidencia
    # supera claramente a la de todos sus divisores).
    max_score = max(s for s, _ in best_by_num.values())
    det_num = min((num for num in best_by_num
                   if best_by_num[num][0] >= 0.82 * max_score),
                  default=declared)
    det_score, det_phase = best_by_num[det_num]
    dec_score = best_by_num.get(declared, (1e-9, 0))[0]
    ratio = det_score / max(dec_score, 1e-9)
    info.update(detected=det_num, phase=det_phase, ratio=round(ratio, 3))
    if det_num != declared and ratio >= min_ratio:
        barlen = det_num * tpb
        ph = det_phase % barlen
        mid.timesig_map = [(0, det_num, 4)] if ph == 0 else [(ph, det_num, 4)]
        info["changed"] = True
    return info


def choose_transposition(notes: List[Note], max_shift: int = 6,
                          per_semitone_cost: float = 0.015,
                          min_gain: float = 0.05) -> int:
    """Elige la transposicion (en semitonos, -6..+6) que minimiza la carga
    de teclas negras ponderada por duracion, con una pequeña penalizacion
    por cada semitono de desplazamiento (para preferir quedarse cerca del
    registro original) y exigiendo una ganancia clara (min_gain) antes de
    tocar nada. Cambiar Fa menor (52% de teclas negras) por La menor (6%)
    es la mayor ganancia de facilidad de un arreglo manual tipico y la
    resta de notas no puede reproducirla."""
    tot = sum(max(1, n.end - n.start) for n in notes)
    if tot <= 0:
        return 0

    def black_frac(sh: int) -> float:
        return sum(max(1, n.end - n.start) for n in notes
                   if (n.pitch + sh) % 12 in _BLACK_PCS) / tot

    lo = min(n.pitch for n in notes)
    hi = max(n.pitch for n in notes)
    base = black_frac(0)
    best_sh, best_cost = 0, base
    for sh in range(-max_shift, max_shift + 1):
        if sh == 0 or lo + sh < 21 or hi + sh > 108:
            continue
        cost = black_frac(sh) + per_semitone_cost * abs(sh)
        if cost < best_cost:
            best_cost, best_sh = cost, sh
    if best_sh != 0 and base - black_frac(best_sh) < min_gain:
        return 0
    return best_sh


def _transpose_events(mid: MidiData, shift: int):
    for trk in mid.tracks:
        for ev in trk.events:
...
def _nearest_octave(pc: int, center: int) -> int:
    p = pc % 12
    k = int(round((center - p) / 12.0))
    return int(min(108, max(21, p + 12 * k)))


def _closed_voicing(pcs: List[int], center: int) -> List[int]:
    """Coloca los pitch-classes de un acorde en posicion cerrada (span <=
    octava) alrededor de `center`, para que la mano izquierda no tenga que
    abrirse ni desplazarse: es la disposicion que usa cualquier arreglo
    pedagogico de patron bajo+acorde."""
    out = sorted({_nearest_octave(pc, center) for pc in pcs})
    guard = 0
    while len(out) > 1 and out[-1] - out[0] > 12 and guard < 8:
        out = sorted(set([out[0] + 12] + out[1:]))
        guard += 1
    if out:
        m = sum(out) / len(out)
        while m < center - 8:
...
def _mk_arr_rec(pitch: int, start: int, end: int, tc: "TimeContext",
                 role: str, vel: int) -> "NoteRec":
    n = Note(pitch=int(min(108, max(21, pitch))), start=start,
             end=max(start + 1, end), vel=vel, channel=1)
    return NoteRec(note=n, hand="lh", bar=tc.bar(start), voice_role=role,
                   function="CT", weight=1.0, floor_protected=True,
                   is_octave_dup=False, candidate=False, orig_id=-2)


def retexture_accompaniment(records: List["NoteRec"], spans: List[ChordSpan],
                             tc: "TimeContext", tpb: int, num: int, target: int,
                             exclude_bars: Optional[Set[int]] = None
                             ) -> Tuple[List["NoteRec"], int, int]:
    """[la carencia conceptual mas grande del motor v3] Sustituye la mano
    izquierda por un patron de acompañamiento REGULAR generado desde el
    analisis armonico (ChordSpan) ya calculado: bajo (fundamental) en el
    tiempo 1 y acordes cerrados en los tiempos restantes (vals en 3/4,
    bajo-acorde en 2/4 y 4/4), con conduccion de voces suave entre compases.
    No es una resta: puede incluso AÑADIR notas — la facilidad viene de la
    regularidad del patron, la posicion cerrada y la estabilidad de la mano,
    exactamente como en una simplificacion manual. La densidad del patron
    depende del target (menos voces y menos ataques en grados bajos). Los
    compases vacios de MI en el original conservan su silencio, y los
    compases en exclude_bars (protegidos/suavizados) se dejan tal cual.
    Devuelve (records, notas_generadas, notas_sustituidas)."""
    lh = [r for r in records if r.hand == "lh"]
    others = [r for r in records if r.hand != "lh"]
    if not lh or not spans:
        return records, 0, 0
    exclude_bars = exclude_bars or set()

    med = sorted(r.note.pitch for r in lh)[len(lh) // 2]
    chord_center = int(min(62, max(50, med + 4)))
    # el bajo vive justo debajo de los acordes (una octava escasa), como en
    # cualquier patron de vals pedagogico: minimiza el vaiven de la mano
    # (el factor 'saltos') en lugar de bajar a la region del bajo original
    bass_center = max(36, chord_center - 12)
    n_chord_voices = 2 if target <= 2 else 3

    last = max(r.note.end for r in records)
    n_bars = max(1, tc.bar(last - 1))
    new_lh: List[NoteRec] = []
    kept_orig: List[NoteRec] = []
    n_replaced = 0
    prev_center = chord_center

    for bar in range(1, n_bars + 1):
        t0, t1 = tc.bar_range_ticks(bar)
        bar_lh = [r for r in lh if t0 <= r.note.start < t1]
        if bar in exclude_bars:
            kept_orig.extend(bar_lh)
            continue
        n_replaced += len(bar_lh)
        # respetar los silencios reales del original: si la MI no sonaba en
        # este compas, no inventar acompañamiento
        if not any(r.note.start < t1 and r.note.end > t0 for r in lh):
            continue
        sp0 = chord_at(spans, tc.beat(t0) + 0.01)
        if sp0 is None or sp0.root is None:
            kept_orig.extend(bar_lh)
            n_replaced -= len(bar_lh)
            continue
        nb = max(1, int(round((t1 - t0) / tpb)))

        # bajo: fundamental en el tiempo 1 (compas entero en grados bajos)
        bass_pitch = _nearest_octave(sp0.root, bass_center)
        bass_end = t1 if target <= 2 else min(t1, t0 + tpb)
        new_lh.append(_mk_arr_rec(bass_pitch, t0, bass_end, tc, "bass", 78))

        # acordes cerrados en el resto de los tiempos, segun target y metro
        if target <= 2:
            chord_beats = [1] if nb >= 2 else []
            chord_end_mode = "hold"          # un solo acorde sostenido
        elif nb >= 4 and target <= 4:
            chord_beats = [1, 3]
            chord_end_mode = "beat2"          # blancas
        else:
            chord_beats = list(range(1, nb))
            chord_end_mode = "beat"           # negras (patron de vals)

        for b in chord_beats:
            bt = t0 + b * tpb
            if bt >= t1:
                continue
            spb = chord_at(spans, tc.beat(bt) + 0.01) or sp0
            pcs = sorted(chord_tone_pcs(spb))
            if not pcs:
                continue
            voicing = _closed_voicing(pcs, prev_center)
            if len(voicing) > n_chord_voices:
                voicing = voicing[-n_chord_voices:]
            while len(voicing) > 1 and voicing[-1] - voicing[0] > 10:
                voicing = voicing[1:]  # nunca exigir mas de una septima justa
            # el acorde nunca invade el registro del bajo
            voicing = [p for p in voicing if p >= bass_pitch + 3] or voicing[-1:]
            # conduccion de voces SIN deriva: el centro sigue al acorde
            # anterior pero siempre dentro de la ventana del registro de
            # acompañamiento (sin esto, la ventana movil de _closed_voicing
            # puede acumular deriva compas a compas hasta el limite del teclado)
            prev_center = int(min(chord_center + 4, max(chord_center - 4,
                                   sum(voicing) / len(voicing))))
            if chord_end_mode == "hold":
                c_end = t1
            elif chord_end_mode == "beat2":
                c_end = min(t1, bt + 2 * tpb)
            else:
                c_end = min(t1, bt + tpb)
            for p in voicing:
                new_lh.append(_mk_arr_rec(p, bt, c_end, tc, "inner", 64))

    out = sorted(others + kept_orig + new_lh, key=lambda r: (r.note.start, r.note.pitch))
    return out, len(new_lh), n_replaced


def thin_rh_to_melody(records: List["NoteRec"], tc: "TimeContext", target: int,
                       exclude_bars: Optional[Set[int]] = None
                       ) -> Tuple[List["NoteRec"], int]:
    """Adelgaza la mano derecha al estilo de un arreglo manual: la MD lleva
    la MELODIA sola (target <= 4) o melodia + un refuerzo puntual solo en
    parte fuerte y cercano a la melodia (target 5). A partir de 6 no toca
    nada. A diferencia del voicing-cap funcional del motor, aqui la
    jerarquia es explicita: la linea melodica manda y las voces internas de
    la MD son siempre las primeras en salir."""
    if target >= 6:
        return records, 0
    exclude_bars = exclude_bars or set()
    mel_at: Dict[int, int] = {}
    for r in records:
        if r.hand == "rh" and r.voice_role == "mel":
            mel_at[r.note.start] = max(mel_at.get(r.note.start, 0), r.note.pitch)
    out: List[NoteRec] = []
    removed = 0
    for r in records:
        if r.hand != "rh" or r.voice_role == "mel" or r.bar in exclude_bars \
           or r.floor_protected:
            out.append(r)
            continue
        if target == 5:
            mp = mel_at.get(r.note.start)
            on_downbeat = tc.beat_in_bar(r.note.start, 0) < 0.26
            if on_downbeat and mp is not None and 0 < mp - r.note.pitch <= 12:
                out.append(r)
                continue
        removed += 1
    return out, removed
...
def normalize_melody_durations(records: List["NoteRec"], tc: "TimeContext",
                                max_legato_beats: float = 2.0) -> List["NoteRec"]:
    """Limpia los artefactos de duracion de un MIDI 'de interpretacion'
    (duraciones tipo 0.77 o 1.23 tiempos): en la MD, liga cada nota hasta el
    siguiente ataque de la mano si el hueco era pequeño, recorta solapes con
    el ataque siguiente y redondea el final a la rejilla compuesta
    (binaria+tresillos). El patron regenerado de la MI ya nace con
    duraciones limpias, asi que solo hace falta en la MD."""
    tpb = tc.tpb
    rh = sorted([r for r in records if r.hand == "rh"],
                key=lambda r: (r.note.start, -r.note.pitch))
    onsets = sorted({r.note.start for r in rh})
    for r in rh:
        i = bisect.bisect_right(onsets, r.note.start)
        nxt = onsets[i] if i < len(onsets) else None
        if nxt is not None:
            if r.note.end > nxt:
                r.note.end = nxt                     # sin solapes en la MD
            elif 0 < nxt - r.note.end <= tpb // 2 and nxt - r.note.start <= max_legato_beats * tpb:
                r.note.end = nxt                     # legato: cerrar micro-huecos
        dur = max(tpb // 4, r.note.end - r.note.start)
        snapped = _snap_composite(r.note.start + dur, max(1, tpb // 4), tpb)
        if snapped > r.note.start:
            r.note.end = snapped
    return records
...
def _snap_composite(tick: int, grid_ticks: int, tpb: int) -> int:
    """Ajusta un tick a la rejilla COMPUESTA: el punto mas cercano entre la
    rejilla binaria pedida (grid_ticks) y la rejilla de tresillos (tpb/3).
    Sin esto, cuantizar una melodia de tresillos (IOI = tpb/3) a corcheas
    binarias destruye precisamente el ritmo caracteristico de la pieza
    (fallo observado frente a la simplificacion manual de referencia). La
    rejilla ternaria solo participa mientras la binaria siga siendo fina
    (grid_ticks < tpb): en los peldaños gruesos ya se busca simplificar el
    ritmo de verdad y ahi los tresillos si deben desaparecer."""
    q = max(0, int(round(tick / float(grid_ticks))) * grid_ticks)
    if grid_ticks < tpb:
        g3 = max(1, tpb // 3)
        q3 = max(0, int(round(tick / float(g3))) * g3)
        if abs(q3 - tick) < abs(q - tick):
            return q3
    return q
...
def _quantize_hand(recs: List["NoteRec"], tc: "TimeContext", grid_ticks: int,
    ...
    for ev in events:
        slot = _snap_composite(ev.start, grid_ticks, tc.tpb)
...
def apply_grid_fallback_until_target(records: List["NoteRec"], tc: "TimeContext",
                                      tpb: int, target_grade: int,
                                      exclude_bars: Optional[Set[int]] = None) -> Tuple[List["NoteRec"], int, bool, int]:
    """Aplica la escala de rejillas progresivamente (cada peldaño parte del
    resultado del anterior, nunca de las notas originales) hasta que el
    grado <= target_grade o se agote la escala. Devuelve
    (records, grado_alcanzado, objetivo_cumplido, nº_de_peldaños_usados).

    exclude_bars: si se pasa (compases protegidos/suavizados por
    --protect-bars/--soften-bars), el criterio de parada de CADA peldaño se
    mide EXCLUYENDO esos compases del calculo — sin esto, un compas
    protegido/suavizado que por si solo siga marcando un grado alto impide
    que el bucle detecte nunca el exito, y agota TODA la escala recortando
    el resto de la pieza mucho mas de lo necesario (bug real detectado al
    combinar --protect-bars/--soften-bars con targets exigentes)."""
    cur = records

    def _grade(recs):
        if exclude_bars:
            return grade_excluding_bars(recs, tc, exclude_bars)
...
def quantize_light(records: List["NoteRec"], tc: "TimeContext", grid_ticks: int) -> List["NoteRec"]:
    ...
    for r in records:
        if r.floor_protected:
            continue
        dur = max(1, r.note.end - r.note.start)
        q = _snap_composite(r.note.start, grid_ticks, tc.tpb)
        r.note.start = q
...
def combined_simplify(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
                       ...
                       protect_bars: Optional[List[int]] = None,
                       soften_bars: Optional[List[int]] = None,
                       soften_span: int = 10, soften_leap: int = 7,
                       soften_stretch: float = 1.25,
                       auto_meter: bool = True, auto_transpose: bool = True,
                       protect_melody: bool = True, retexture: bool = True,
                       retexture_max_grade: int = 6) -> Dict:
    """Regiones de interes, independientes entre si y combinables en la
    misma llamada:
       protect_bars — [OPCION A: excepcion] esos compases quedan
                       protegidos byte a byte (floor_protected desde antes
                       del greedy: pre-pasada, greedy, optimizador y rejilla
                       nunca los tocan). El grado por target se decide
                       EXCLUYENDO esos compases del calculo; se informa
                       ademas el grado real incluyendolos.
       soften_bars  — [OPCION B: suavizado] esos compases se pasan por
                       soften_climax (voicing/saltos mas estrictos + ataques
                       algo mas espaciados) y LUEGO quedan protegidos igual
                       que protect_bars, para que la rejilla no les revierta
                       el suavizado si el resto de la pieza aun necesita mas
                       reduccion. El grado SI los incluye (no hay excepcion
                       que reportar): son una version mas simple pero fiel
                       de esos compases, no el original sin tocar.
    Un mismo compas no deberia estar en las dos listas a la vez."""
    protect_set: Set[int] = set(protect_bars or [])
    soften_set: Set[int] = set(soften_bars or [])
    overlap = protect_set & soften_set
    if overlap:
        raise ValueError(f"compases en protect_bars y soften_bars a la vez: {sorted(overlap)}")

    result = reduce_graded(midi_path, target_grades, floor_level=floor_level, split=split,
                            pre_simplify=use_transforms,
                            climax_bars=protect_set if protect_set else None,
                            auto_meter=auto_meter, auto_transpose=auto_transpose,
                            protect_melody=protect_melody)
    ...
    if soften_set:
        for t in result["level_snapshots"]:
            softened = soften_climax(
                result["level_snapshots"][t], tc, soften_set,
                max_span=soften_span, max_leap=soften_leap, stretch=soften_stretch)
            mark_climax_protected(softened, soften_set)
            result["level_snapshots"][t] = softened

    # exclude_for_grade: SOLO protect_set se saca del grado que se reporta
    #   (soften_set cuenta de verdad, es una version simplificada real, no
    #   una excepcion).
    # stop_bars: protect_set + soften_set. Ambos quedan floor_protected en
    #   este punto -> la rejilla y el optimizador jamas los tocan, asi que
    #   NINGUNO de los dos puede seguir aportando reduccion. Usar esto (no
    #   solo protect_set) como criterio de PARADA evita el bug real que
    #   viste: sin esto, un compas suavizado-y-congelado que por si solo
    #   siga marcando un grado alto impedia detectar el exito y la rejilla
    #   agotaba toda la escala recortando el resto de la pieza de mas.
    exclude_for_grade = protect_set
    stop_bars = protect_set | soften_set

    ts_num = mid.timesig_map[0][1] if mid.timesig_map else 4

    levels: Dict[int, LevelResult] = {}
    for t in sorted(set(target_grades), reverse=True):
        snap = result["level_snapshots"][t]

        # [ARREGLO] retexturizado estilo arreglista: MI regenerada como
        # patron regular desde la armonia + MD adelgazada a la melodia +
        # duraciones normalizadas. Se aplica ANTES de decidir metodo porque
        # su ganancia (regularidad, posicion cerrada, mano estable) es
        # justamente lo que la resta estructural no puede conseguir. Los
        # compases protegidos/suavizados quedan fuera.
        arr_tag = ""
        lh_irr_snap = _irregularity([r.note for r in snap if r.hand == "lh"])
        if retexture and t <= retexture_max_grade and lh_irr_snap >= 0.22:
            arr = _clone_records(snap)
            arr, n_gen, n_repl = retexture_accompaniment(
                arr, result["spans"], tc, mid.tpb, ts_num, t,
                exclude_bars=stop_bars or None)
            arr, n_thin = thin_rh_to_melody(arr, tc, t, exclude_bars=stop_bars or None)
            arr = normalize_melody_durations(arr, tc)
            arr = _sort_records(arr)
            g_arr = (grade_excluding_bars(arr, tc, stop_bars) if stop_bars
                      else _records_grade(arr, tc))
            g_snap = (grade_excluding_bars(snap, tc, stop_bars) if stop_bars
                       else _records_grade(snap, tc))
            if n_gen > 0 and g_arr <= g_snap:
                snap = arr
                arr_tag = (f"+arreglo (MI regenerada: {n_gen} notas de patron, "
                           f"{n_repl} sustituidas; MD melodica: -{n_thin})")

        achieved_stop = (grade_excluding_bars(snap, tc, stop_bars) if stop_bars
                          else _records_grade(snap, tc))
        ...
        if achieved_stop <= t:
            ...
            achieved_t = (grade_excluding_bars(transformed, tc, stop_bars) if stop_bars
                          else achieved_t_incl)
            if achieved_t <= t:
                ...
                reduced, _achieved2, ok_real, gsteps = apply_grid_fallback_until_target(
                    snap, tc, mid.tpb, t, exclude_bars=stop_bars or None)
            ...
            reduced, _achieved2, ok, gsteps = apply_grid_fallback_until_target(
                snap, tc, mid.tpb, t, exclude_bars=stop_bars or None)
            method = "estructural+rejilla" if ok else f"estructural+rejilla (limite tras {gsteps} peldaños)"
            chosen = reduced

        # grado OFICIAL reportado: excluye solo protect_set (excepcion de
        # verdad); soften_set cuenta, porque es contenido real, simplificado
        # pero real.
        achieved_final = (grade_excluding_bars(chosen, tc, exclude_for_grade) if exclude_for_grade
                           else _records_grade(chosen, tc))
        ...
        tags = []
        if protect_set:
            tags.append(f"protegidos: {sorted(protect_set)}")
        if soften_set:
            tags.append(f"suavizados: {sorted(soften_set)}")
        if tags:
            method += "  [" + "; ".join(tags) + "]"
        achieved_real = _records_grade(chosen, tc) if protect_set else achieved_final
    ...
    return {"mid": mid, "tc": tc, "levels": levels, "result": result,
            "protect_bars": sorted(protect_set), "soften_bars": sorted(soften_set)}
...
def print_combined_report(midi_path: str, out: Dict):
    ...
    mf = out["result"].get("meter_fix")
    if mf and mf.get("changed"):
        print(f"  metro corregido: {mf['declared']}/4 declarado -> {mf['detected']}/4 detectado "
              f"(fase {mf['phase']} ticks, evidencia x{mf['ratio']:.2f}) — todo el analisis "
              f"por compas y el MIDI exportado usan el metro detectado")
    tsh = out["result"].get("transpose_shift", 0)
    if tsh:
        print(f"  transposicion automatica: {tsh:+d} semitonos "
              f"(teclas negras: {out['result']['black_keys_before']:.0%} -> "
              f"{out['result']['black_keys_after']:.0%})")
    nmel = out["result"].get("n_melody_protected", 0)
    if nmel:
        print(f"  melodia protegida: {nmel} notas intocables para greedy/rejilla "
              f"(solo la resolucion de adornos pudo simplificarla antes)")
    ...
    if out.get("protect_bars"):
        print(f"  compases protegidos (excepcion, tal cual el original): {out['protect_bars']}")
    if out.get("soften_bars"):
        print(f"  compases suavizados (mismo contenido, mas tocable): {out['soften_bars']}")
...
def main():
    ap = argparse.ArgumentParser(
        ...
        description="Simplifica un MIDI de piano a un nivel de dificultad pedagogica pedido: "
                     ...
                     "con opcion de proteger (tal cual) o suavizar (mas fiel al original, algo mas "
                     "tocable) compases concretos — util para clímax u otros pasajes densos que la "
                     "rejilla distorsiona mas de la cuenta.")
    ...
    ap.add_argument("--protect-bars", type=int, nargs="+",
                     help="[OPCION A: excepcion] compas(es) que se dejan EXACTAMENTE como en el "
                          "original (numeracion interna, la misma que devuelve --report-bars) y se "
                          "excluyen del calculo de grado. Combinable con --soften-bars (compases "
                          "distintos).")
    ap.add_argument("--soften-bars", type=int, nargs="+",
                     help="[OPCION B: suavizado] compas(es) que se simplifican con criterio mas "
                          "estricto que el resto (voicing/saltos/espaciado de ataques) en vez de "
                          "pasar por la rejilla de respaldo, para sonar mas parecido al original. "
                          "Cuentan en el calculo de grado (no son una excepcion). Combinable con "
                          "--protect-bars (compases distintos).")
    ap.add_argument("--soften-span", type=int, default=10,
                     help="[solo --soften-bars] extension maxima (semitonos) tras close_voicings "
                          "dentro de los compases suavizados (def. 10)")
    ap.add_argument("--soften-leap", type=int, default=7,
                     help="[solo --soften-bars] salto maximo (semitonos) tras reduce_melodic_leaps "
                          "dentro de los compases suavizados (def. 7)")
    ap.add_argument("--soften-stretch", type=float, default=1.25,
                     help="[solo --soften-bars] factor de separacion entre ataques consecutivos "
                          "dentro de los compases suavizados (def. 1.25 = 25%% mas espacio)")
    ap.add_argument("--no-meter-detect", action="store_true",
                     help="[v4] no corregir el metro declarado en el MIDI aunque el perfil de "
                          "acentos indique otro (p.ej. un vals en 3/4 exportado como 4/4)")
    ap.add_argument("--no-transpose", action="store_true",
                     help="[v4] no transportar automaticamente a la tonalidad con menos teclas "
                          "negras (se conserva la tonalidad original)")
    ap.add_argument("--no-protect-melody", action="store_true",
                     help="[v4] permitir que greedy/rejilla eliminen notas de la linea melodica "
                          "(comportamiento v3; necesario si se piden grados muy bajos a toda costa)")
    ap.add_argument("--no-retexture", action="store_true",
                     help="[v4] no regenerar la mano izquierda como patron de acompañamiento "
                          "regular ni adelgazar la MD a la melodia")
    ap.add_argument("--retexture-max-grade", type=int, default=6,
                     help="[v4] target-grade maximo al que se aplica el retexturizado (def. 6; "
                          "por encima se respeta la textura original)")
    ap.add_argument("--report-bars", action="store_true",
                     help="solo imprime el grado por compas (util para localizar clímax o pasajes "
                          "problematicos) y sale")
    args = ap.parse_args()
    ...
    out = combined_simplify(args.midi, args.target_grade, floor_level=args.floor_level, split=args.split,
                             ...
                             protect_bars=args.protect_bars, soften_bars=args.soften_bars,
                             soften_span=args.soften_span, soften_leap=args.soften_leap,
                             soften_stretch=args.soften_stretch,
                             auto_meter=not args.no_meter_detect,
                             auto_transpose=not args.no_transpose,
                             protect_melody=not args.no_protect_melody,
                             retexture=not args.no_retexture,
                             retexture_max_grade=args.retexture_max_grade)
