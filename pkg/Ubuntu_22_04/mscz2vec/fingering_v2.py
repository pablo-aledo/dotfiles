#!/usr/bin/env python3
"""
fingering_v2.py — Generador automático de digitación pianística a partir de MIDI.
...
Uso:
    python fingering_v2.py archivo.mid
    ...
    python fingering_v2.py archivo.mid --confidence      # muestra puntuación de confianza
"""
...
def load_midi(path: str, right_track: int = 0, left_track: int = 1,
              auto_split: bool = False) -> tuple[pretty_midi.PrettyMIDI,
                                                  list[INote], list[INote],
                                                  TempoMap]:
    """
    Carga un fichero MIDI y devuelve (pm, rh_seq, lh_seq, tmap).

    Si auto_split=True o sólo hay 1 instrumento, usa separación inteligente.
    """
    ...
    rh_seq = _build_note_seq(rh_raw, tmap, "right") if rh_raw else []
    lh_seq = _build_note_seq(lh_raw, tmap, "left")  if lh_raw else []
    return pm, rh_seq, lh_seq, tmap
...
def build_result(midi_path: str, tmap: TempoMap,
                 rh_seq: list[INote], lh_seq: list[INote]) -> FingeringResult:
    """Construye el resultado estructurado como FingeringResult."""
    all_measures = set()
    for n in rh_seq + lh_seq:
        all_measures.add(n.measure)

    pm_ts = None
    notes_out = []
    for n in sorted(rh_seq + lh_seq, key=lambda n: (n.measure, n.beat, n.hand, n.pitch)):
        notes_out.append({
            "measure":    n.measure,
            "beat":       n.beat,
            "hand":       n.hand,
    ...
    for m in sorted(by_measure):
        ...
        for hand_key, hand_label in hands:
            ...
            for n in sorted(notes, key=lambda x: (x["beat"], x["pitch"])):
                ...
                print(f"    {n['note']:<5} — {f} ({name}){ambig}{chord}{conf_str}")
        print()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Genera digitación pianística a partir de un fichero MIDI (v2)."
    )
    ...
    try:
        pm, rh_seq, lh_seq, tmap = load_midi(
            args.midi,
    ...
    if args.left_only:  rh_seq = []
    if args.right_only: lh_seq = []
    ...
    result = build_result(args.midi, tmap, rh_seq, lh_seq)
    ...
    if args.xml:
        export_musicxml(result, rh_seq, lh_seq, args.xml)
