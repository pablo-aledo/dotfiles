...
def reduce_graded(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
                   ...
                   max_voices_per_chord: Optional[int] = None) -> dict:
...
def combined_simplify(midi_path: str, target_grades: List[int], floor_level: str = "ursatz",
    ...
    for t in sorted(set(target_grades), reverse=True):
        ...
        if achieved <= t:
            levels[t] = LevelResult(t, achieved, "estructural", len(snap), n_original, snap)
        else:
            reduced, achieved2, ok, steps = apply_grid_fallback_until_target(snap, tc, mid.tpb, t)
            method = "estructural+rejilla" if ok else f"estructural+rejilla (limite tras {steps} peldaños)"
            levels[t] = LevelResult(t, achieved2, method, len(reduced), n_original, reduced)
...
def print_combined_report(midi_path: str, out: Dict):
    ...
    print(f"\n{'═'*78}\n  COMBINED SIMPLIFIER v1.1 (autocontenido) — {midi_path}\n{'═'*78}")
