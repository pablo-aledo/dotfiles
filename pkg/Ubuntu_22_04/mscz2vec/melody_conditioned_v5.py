...
║  Cada nota se condiciona al acorde activo en ese momento: el motor sabe     ║
║  en todo instante qué acorde suena y toma decisiones en consecuencia.       ║
║                                                                              ║
║  Nueve motores de generación, 16 perfiles emocionales, post-procesado       ║
║  armónico, y un compositor de estructuras formales completas.                ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  MOTORES (--engine)                                                          ║
║  ──────────────────                                                          ║
║  chord_guided  Markov de 2º orden condicionado nota a nota al acorde.       ║
║                Pesos = función_armónica × perfil × tensión_actual.          ║
║                Sin entrenamiento. Con --corpus usa MIDIs reales.            ║
║                                                                              ║
║  grammar       Elaboración Schenkerian en tres capas: chord tones           ║
║                estructurales → notas de paso → bordaduras/appoggiaturas.    ║
║                Determinista. Melodías cantables y bien formadas.            ║
║                                                                              ║
║  search        Beam search A* (beam=5). Coste local: penaliza avoid notes   ║
║                en tiempo fuerte (+5), fuera de escala (+2.5), saltos >12;  ║
║                premia chord tones (-1.5). Garantías locales por nota.       ║
║                                                                              ║
║  genetic       Algoritmo genético (pop=20, gen=30). Fitness: armonía por   ║
║                acorde (30%), contorno (20%), suavidad (20%), variedad       ║
║                rítmica (10%), rango (10%), clímax ajustado al perfil (10%). ║
║                                                                              ║
║  lstm          LSTM (~114K params) entrenada sobre corpus propio.           ║
║                Piano roll cuantizado a semicorchea. Condicionamiento        ║
║                beat-a-beat via chord_vector[16]. Diseñado para CPU.         ║
║                Requiere --train previo o --model.                           ║
║                                                                              ║
║  motif         Construye por transformaciones de un motivo semilla:         ║
║                identity, sequence_up/down, inversion, retrograde,           ║
║                augmentation, diminution, variation. Motivo del usuario      ║
║                (--motif) o generado automáticamente. Coherencia temática.  ║
║                                                                              ║
║  bayes         N-gram orden 3 con suavizado Kneser-Ney sobre corpus.       ║
║                Vocabulario: (pitch_class, dur_bucket, chord_func).          ║
║                Estimación razonada para transiciones no vistas.             ║
║                Sin torch. Con --corpus real; sin él usa fallback sintético. ║
║                                                                              ║
║  tension_arc   Planifica de atrás hacia adelante por frase (--phrase-bars). ║
║                Nota de resolución → nota de clímax → relleno del camino.   ║
║                Clímax posicionado según PROFILE_CLIMAX_POS del perfil.     ║
║                Narrativa armónica explícita con destino audible.            ║
║                                                                              ║
║  interpolate   Interpolación lineal entre dos perfiles en cinco             ║
║                dimensiones: velocidades, pesos Markov, pesos CT/T/PT/NT,   ║
║                posición del clímax, curva de tensión.                       ║
║                --alpha 0.0 = perfil A puro, 1.0 = perfil B puro.          ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  FORM — COMPOSITOR DE ESTRUCTURAS FORMALES (--form)                          ║
║  ──────────────────────────────────────────────────                          ║
║  Genera una obra completa con arquitectura formal. Orquesta múltiples       ║
║  instancias del generador (una por sección) y ensambla en un único MIDI     ║
║  con marcadores de sección visibles en cualquier DAW.                       ║
║                                                                              ║
║  Formas predefinidas:                                                        ║
║    ABA          ternaria clásica (40% + 30% + 30%)                          ║
║    ABAB         binaria repetida                                             ║
║    AABA         forma canción de jazz/pop (32 compases)                     ║
║    ABACABA      rondó con dos episodios                                     ║
║    verse_chorus V C V C B C (pop/rock con puente)                           ║
║    sonata       E T D R C (exposición, transición, desarrollo,              ║
║                 recapitulación, coda)                                        ║
║                                                                              ║
║  Formas libres: "A B A' C A" o "ABACA" (A' = variación de A)               ║
║                                                                              ║
║  Por sección:                                                                ║
║    A / A'  Tonalidad y perfil base. A' varía el penúltimo acorde.          ║
║    B       Perfil complementario. Tonalidad: --form-key-b                   ║
║    C / D   Contraste máximo. Tonalidad: --form-key-c                        ║
║    Repeticiones exactas reutilizan el resultado cacheado.                   ║
║                                                                              ║
║  Tonalidades: same · relative · dominant · subdominant ·                    ║
║               parallel · mediant · submediant                               ║
║                                                                              ║
║  --form-bridge-bars N  Puentes modulantes entre secciones con distinta      ║
║  tonalidad (tension_arc + perfil tense sobre el acorde pivot).              ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  PERFILES EMOCIONALES (--profile)                                            ║
║  ─────────────────────────────────                                           ║
║  Cada perfil configura 7 capas: modo de escala, curva de tensión, pesos    ║
║  de intervalo Markov, multiplicadores CT/T/PT/NT, contorno global,          ║
║  rango de velocidades y posición del clímax.                                ║
║                                                                              ║
...
║  elegiac      melodic_minor       pp–mf       6ª desc., luto contenido      ║
...
║  POST-PROCESADO                                                              ║
║  ──────────────                                                              ║
║  --fix-avoid    Corrige avoid notes en tiempos fuertes con nearest_         ║
║                 admissible, preservando contorno. Respeta notas de paso     ║
║                 y bordaduras legítimas.                                     ║
║                                                                              ║
║  --ornaments O  appoggiatura (25%+75%) | passing (60%+20%+20%) |           ║
║                 neighbor (50%+25%+25%) | all                                ║
║  --ornament-prob F  Probabilidad por nota (default: 0.35)                   ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  SCORING (selección del mejor candidato)                                     ║
║  ────────────────────────────────────────                                    ║
║    15%  Consonancia con la escala                                            ║
║    20%  Armonía por acorde activo (CT=1.0 T=0.7 PT=0.5 NT=0.1)             ║
║         Avoid notes en tiempo fuerte penalizadas ×0.2                       ║
║    15%  Suavidad de movimiento                                               ║
║    15%  Conformidad con el contorno del perfil                              ║
║    10%  Rango melódico razonable (óptimo: 12 semitonos)                     ║
║    10%  Variedad rítmica                                                     ║
║     8%  Arco dinámico (desv. estándar de velocidades)                       ║
║     7%  Posición del clímax (ajustada al perfil)                            ║
║  Con --verbose: score por acorde + lista de colisiones.                     ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  LSTM — DESCRIPCIÓN TÉCNICA                                                  ║
║  ──────────────────────────                                                  ║
║  Piano roll: 16 pasos/compás (4/4). Token: 0–108 pitch, 109 sil, 110 hold. ║
║  chord_vector[16]: [0:12] pitch classes, [12:16] harm_func one-hot.        ║
║  Arquitectura: Embedding(111,32)→concat→LSTM(48→128)→Linear(128→111)       ║
║  Entrenamiento: truncated BPTT, chunks 32, batch 16, ~1MB RAM/chunk.       ║
║  Augmentación: ±5 st transposición (×11) + ventanas solapadas stride 16.   ║
║  Generación: bias de contorno + escala + sin hold en tiempo fuerte.        ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  EJEMPLOS DE USO                                                             ║
...
║                                                                              ║
║  ── chord_guided ────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py                                             ║
║      --chords "Am:2 G:2 F:2 E7:2" --key Am --bars 8 --profile melancholic  ║
║                                                                              ║
║  python melody_conditioned_v5.py --corpus ./midis/                           ║
║      --chords "C:4 Am:4 F:4 G:4" --key C --bars 16 --profile serene        ║
║      --fix-avoid --ornaments all --candidates 5                              ║
║                                                                              ║
║  ── grammar ─────────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine grammar                            ║
║      --chords "Am:4 G:4 F:4 E:4" --key Am --mode phrygian                  ║
║      --profile mysterious --contour descending --bars 8                     ║
║                                                                              ║
║  ── search ──────────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine search                             ║
║      --chords "Cm:2 Ab:2 Eb:2 Bb:2" --key Cm --profile brooding --fix-avoid║
║                                                                              ║
║  ── genetic ─────────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine genetic                            ║
║      --chords "D:4 A:4 Bm:4 G:4" --key D --profile triumphant --candidates 5║
║                                                                              ║
║  ── lstm ────────────────────────────────────────────────────────────────   ║
║  # Preprocesar (una vez)                                                     ║
║  python melody_conditioned_v5.py --preprocess --corpus ./midis/              ║
║  # Entrenar                                                                  ║
║  python melody_conditioned_v5.py --train --dataset corpus.npz --epochs 30   ║
║  # Generar                                                                   ║
║  python melody_conditioned_v5.py --engine lstm --model lstm.pt              ║
║      --chords "Am:2 G:2 F:2 E7:2" --key Am --profile melancholic            ║
║  # Todo en uno                                                               ║
║  python melody_conditioned_v5.py --engine lstm --train --corpus ./midis/    ║
║      --chords "Am:2 G:2 F:2 E7:2" --key Am --bars 8                        ║
║                                                                              ║
║  ── motif ───────────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine motif                              ║
║      --motif "D4:0.5 F#4:0.5 A4:1 F#4:0.5 D4:0.5"                         ║
║      --chords "D:4 A:4 Bm:4 G:4" --key D --bars 16 --profile heroic        ║
║                                                                              ║
║  ── bayes ───────────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine bayes --corpus ./midis/           ║
║      --chords "Am:2 G:2 F:2 E7:2" --key Am --bars 16 --profile melancholic ║
║                                                                              ║
║  ── tension_arc ─────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine tension_arc                        ║
║      --chords "Cm:4 Ab:4 Eb:4 Bb:4" --key Cm --profile tense --phrase-bars 4║
║                                                                              ║
║  ── interpolate ─────────────────────────────────────────────────────────   ║
║  python melody_conditioned_v5.py --engine interpolate                        ║
║      --profile flamenco --profile-b tanguero --alpha 0.5                    ║
║      --chords "Am:2 G:2 F:2 E7:2" --key Am --bars 16                       ║
║                                                                              ║
║  ── form ────────────────────────────────────────────────────────────────   ║
║  # ABA básico                                                                ║
║  python melody_conditioned_v5.py --form ABA --key Am --profile melancholic  ║
║      --form-chords-a "Am:2 G:2 F:2 E7:2" --form-key-b relative             ║
║      --form-total-bars 48                                                    ║
║                                                                              ║
║  # ABA con puente modulante                                                  ║
║  python melody_conditioned_v5.py --form ABA --key Am --profile flamenco     ║
║      --form-chords-a "Am:2 G:2 F:2 E:2" --form-key-b dominant              ║
║      --form-bridge-bars 2 --form-total-bars 48                              ║
║                                                                              ║
║  # Sonata heroica con motor motif                                            ║
║  python melody_conditioned_v5.py --form sonata --key D --profile heroic     ║
║      --form-chords-a "D:4 A:4 Bm:4 G:4"                                    ║
║      --form-key-b dominant --form-key-c relative                            ║
║      --engine motif --motif "D4:0.5 F#4:0.5 A4:1"                          ║
║      --form-bridge-bars 2 --include-chords --form-total-bars 64             ║
║                                                                              ║
║  # Forma libre flamenco-tango                                                ║
║  python melody_conditioned_v5.py --form "A B A' C A" --key Am              ║
║      --profile flamenco --profile-b tanguero --engine interpolate           ║
║      --alpha 0.5 --form-total-bars 80                                       ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  OPCIONES COMPLETAS                                                          ║
...
║                   motif|bayes|tension_arc|interpolate                        ║
║    --key / --mode / --bars / --tempo / --time-sig / --profile               ║
║    --rhythm       flowing|march|syncopated|baroque|jazz|sparse|dense        ║
║    --contour      auto|arch|ascending|descending|wave|plateau|              ║
║                   inverted|erratic|custom                                   ║
║    --contour-values V   Para --contour custom: "0,0.3,0.8,1,0.5"           ║
║    --tension-curve V    Por compás: "0.2,0.5,0.8,…"                        ║
║    --range L H   Rango MIDI (default: 60 84)                                ║
║    --candidates N / --seed N / --verbose / --dry-run                        ║
║                                                                              ║
║  Acordes:                                                                    ║
║    --chords / --chords-json / --chords-midi                                  ║
║    --chords-midi-track N (def:2) / --chords-midi-channel N (def:1)         ║
║    --chords-midi-window F (def:0.5)                                         ║
║                                                                              ║
║  Corpus: --corpus DIR / --max-files N                                        ║
║                                                                              ║
║  LSTM: --train / --preprocess / --dataset / --model-out / --model           ║
║        --epochs / --hidden / --d-embed / --num-layers / --batch-size        ║
║        --chunk-size / --lr / --temperature / --top-k                        ║
║                                                                              ║
║  Motif:       --motif "C4:0.5 E4:1 G4:0.5"                                  ║
║  Interpolate: --profile-b P / --alpha F (0.0–1.0)                           ║
║  Tension arc: --phrase-bars N (default: 4)                                   ║
║                                                                              ║
║  Form:                                                                       ║
║    --form STR              ABA|ABAB|AABA|ABACABA|verse_chorus|sonata        ║
║                            o libre: "A B A' C A"                            ║
║    --form-total-bars N     Total de compases (default: 64)                  ║
║    --form-engine E         Motor base para las secciones                    ║
║    --form-chords-a/b/c     Progresiones específicas por sección             ║
║    --form-key-b/c SCHEME   same|relative|dominant|subdominant|              ║
║                            parallel|mediant|submediant                      ║
║    --form-bridge-bars N    Compases de puente modulante (default: 0)        ║
║    --form-contour-a/b STR  Contorno por sección                             ║
║    --form-markers          Marcadores de sección en el MIDI                 ║
║                                                                              ║
║  Post-procesado: --ornaments / --ornament-prob / --fix-avoid                ║
║  Exportación:    --output / --include-chords / --export-motif               ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  SALIDAS                                                                     ║
║  ───────                                                                     ║
║  Modo normal:                                                                ║
║    melody_conditioned.mid          Mejor candidato                          ║
║    melody_conditioned_v2.mid       Segundo candidato                        ║
║    melody_conditioned.melody.json  Notas + score + detalle por criterio     ║
║    melody_conditioned.motif.mid    Motivo semilla (con --export-motif)      ║
║                                                                              ║
║  Modo form (--form):                                                         ║
║    output.mid          Obra completa con marcadores de sección              ║
║    output.form.json    label, bars, key, profile, engine, score,            ║
║                        offset_beats por sección                             ║
║                                                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  DEPENDENCIAS                                                                ║
...
║    pip install music21         (opcional: detección de acordes mejorada)    ║
║                                                                              ║
║  Formato del corpus MIDI para LSTM:                                          ║
║    Track 0: metadatos | Track 1: melodía (canal 0) | Track 2: acordes (1)  ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  FORM — COMPOSITOR DE ESTRUCTURAS FORMALES
# ══════════════════════════════════════════════════════════════════════════════

# Formas predefinidas: lista de (label, beats_fraction, profile_modifier)
# beats_fraction = fracción del total de compases asignada a la sección
# profile_modifier = modificación al perfil base (None = sin cambio)
PREDEFINED_FORMS: Dict[str, List[Tuple[str, float, Optional[str]]]] = {
    "ABA":      [("A", 0.40, None), ("B", 0.30, None), ("A", 0.30, None)],
    "ABAB":     [("A", 0.25, None), ("B", 0.25, None),
                 ("A", 0.25, None), ("B", 0.25, None)],
    "ABACABA":  [("A", 0.15, None), ("B", 0.15, None), ("A", 0.10, None),
                 ("C", 0.20, None), ("A", 0.10, None), ("B", 0.15, None),
                 ("A", 0.15, None)],
    "AABA":     [("A", 0.25, None), ("A", 0.25, None),
                 ("B", 0.25, None), ("A", 0.25, None)],
    "verse_chorus": [("V", 0.20, None), ("C", 0.15, None),
                     ("V", 0.20, None), ("C", 0.15, None),
                     ("B", 0.15, None), ("C", 0.15, None)],
    "sonata":   [("E", 0.15, None), ("T", 0.20, None), ("D", 0.25, None),
                 ("R", 0.25, None), ("C", 0.15, None)],
    # E=exposición T=transición D=desarrollo R=recapitulación C=coda
}

# Cómo se transforma el perfil en cada tipo de sección
SECTION_PROFILE_SHIFT: Dict[str, Dict[str, str]] = {
    # Para sección B respecto a sección A: cambio de perfil complementario
    "B": {
        "heroic":      "melancholic",
        "melancholic": "serene",
        "tense":       "serene",
...
@dataclass
class SectionSpec:
    """Especificación de una sección de la forma musical."""
    label:       str           # "A", "B", "C", "V", "E"…
    bars:        int           # compases de la sección
    profile:     str           # perfil emocional
    engine:      str           # motor de generación
    chords:      str           # progresión en texto
    contour:     str           # forma del contorno
    tension:     Optional[List[float]] = None  # curva de tensión explícita
    key:         str = "C"
    mode:        str = "auto"
    is_variation: bool = False  # True si es repetición variada de otra sección
    source_label: Optional[str] = None  # de qué sección es variación


@dataclass
class FormResult:
    """Resultado de la generación formal completa."""
    sections:     List[Tuple[str, MelodyResult]]  # (label, resultado)
    total_bars:   int
    total_notes:  int
    tempo:        int
    key:          str
...
def parse_form_string(form_str: str) -> List[str]:
    """
    Parsea una cadena de forma musical.
    Soporta:
      "ABA"          → ["A", "B", "A"]
      "A B A"        → ["A", "B", "A"]
      "A B A' C A"   → ["A", "B", "A'", "C", "A"]
    ...
    """
    # Si contiene espacios, dividir por espacios
    if " " in form_str.strip():
        return [s.strip() for s in form_str.split() if s.strip()]
    # Si no, parsear carácter a carácter (con soporte de prima ')
    result = []
    i = 0
    while i < len(form_str):
        ...
        if ch.isalpha() or ch.isdigit():
            label = ch
            if i + 1 < len(form_str) and form_str[i+1] == "'":
                label += "'"
                i += 1
            result.append(label)
        i += 1
...
def _transpose_chord_progression(chords_text: str,
                                  semitones: int,
                                  root_pc_key: int,
                                  mode: str) -> str:
    """
    Transpone una progresión de acordes N semitonos.
    Preserva las duraciones.
    """
    if semitones == 0:
        return chords_text
    tokens = chords_text.strip().split()
    result = []
    for tok in tokens:
        if ":" in tok:
            name, beats = tok.split(":", 1)
            ...
            name, beats = tok, "4"
        root_pc, quality = parse_chord_name(name)
        new_root = (root_pc + semitones) % 12
        new_name = NOTE_NAMES[new_root] + quality
        result.append(f"{new_name}:{beats}")
    return " ".join(result)


def _vary_progression(chords_text: str, rng: random.Random,
                       root_pc_key: int, mode: str) -> str:
    """
    Crea una variación de la progresión: sustituye el penúltimo acorde
    por un acorde de sustitución (tritono o relativo).
    """
    tokens = chords_text.strip().split()
    if len(tokens) < 2:
        return chords_text
    # Variar el penúltimo acorde
    idx = -2
    tok = tokens[idx]
    if ":" in tok:
        name, beats = tok.split(":", 1)
        ...
        name, beats = tok, "4"
    root_pc, quality = parse_chord_name(name)
    # Sustitución de tritono (±6 semitonos) o relativo (±3)
    sub = rng.choice([6, -6, 3, -3, 9])
    new_root = (root_pc + sub) % 12
    # Mantener calidad pero añadir 7 para el color
    new_quality = quality if quality else "7"
    new_name = NOTE_NAMES[new_root] + new_quality
    tokens[idx] = f"{new_name}:{beats}"
    return " ".join(tokens)


def _modulate_progression(chords_text: str, from_key: int,
                            to_key: int, mode: str) -> str:
    """
    Crea un puente modulante desde from_key hasta to_key.
    Inserta un acorde pivot y el dominante de la nueva tonalidad.
    """
    # Dominante de la nueva tonalidad
    dom_pc    = (to_key + 7) % 12
    dom_name  = NOTE_NAMES[dom_pc] + "7"
    # Pivot: el IV de from_key que suena como II de to_key
    pivot_pc  = (from_key + 5) % 12
    pivot_name = NOTE_NAMES[pivot_pc] + "m7"
    return f"{pivot_name}:2 {dom_name}:2"


class FormComposer:
    """
    Compositor de estructuras formales completas.

    Orquesta múltiples instancias de ConditionedMelodyGenerator,
    una por sección, y ensambla el resultado en un MIDI continuo.

    Conceptos clave:
      - Sección A: material temático principal. Define el motivo,
        el perfil y la tonalidad base.
      - Sección A' (prima): variación de A — misma estructura pero
        con ornamentación extra, variación de la progresión, o
        transposición de la respuesta final.
      - Sección B: contraste. Perfil complementario, opcionalmente
        una tonalidad vecina (relativa, dominante, subdominante).
      - Sección C: episodio/trío/desarrollo. Máximo contraste.
        Puede modular a una tonalidad más lejana.
      - Transiciones: compases de puente entre secciones que difieren
        en tonalidad (modulación). Opcionales (--bridge-bars N).

    Parámetros de tonalidad por sección:
      Las secciones no-A pueden moverse a:
        "relative" → relativa mayor/menor
        "dominant" → un quinto arriba
        "subdominant" → un cuarto arriba
        "parallel" → mismo grado, modo opuesto
        "same" → misma tonalidad (default)
    """

    # Desplazamiento de tonalidad por tipo de sección y estilo
    KEY_SHIFT: Dict[str, int] = {
        "same":        0,
        "relative":    9,   # relativa menor de mayor = -3 st (o +9)
        "dominant":    7,
        "subdominant": 5,
        "parallel":    0,   # mismo pitch, modo alternado
        "mediant":     4,   # mediante mayor
        "submediant":  9,   # supermediante
    }
    ...
    def __init__(self,
                 ...
                 time_sig: str = "4/4",
                 form: str = "ABA",
                 base_engine: str = "chord_guided",
                 base_profile: str = "serene",
                 base_rhythm: str = "flowing",
                 chords_a: Optional[str] = None,
                 chords_b: Optional[str] = None,
                 chords_c: Optional[str] = None,
                 key_b: str = "same",
                 key_c: str = "relative",
                 bridge_bars: int = 0,
                 contour_a: str = "auto",
        ...
        self.form_string  = form
        self.base_engine  = base_engine
        self.base_profile = base_profile
        self.base_rhythm  = base_rhythm
        self.chords_a     = chords_a
        self.chords_b     = chords_b
        self.chords_c     = chords_c
        self.key_b        = key_b
        self.key_c        = key_c
        self.bridge_bars  = bridge_bars
        ...
        # Parsear tonalidad y modo base
        self.root_pc, detected_mode = parse_key(key)
        self.mode = detected_mode if mode == "auto" else mode
        if self.mode == "auto":
            self.mode = PROFILE_TO_MODE.get(base_profile, "major")

        # Parsear la forma
        self.sections_labels = parse_form_string(form)

        # Verificar si es una forma predefinida
        if form in PREDEFINED_FORMS:
            self._form_template = PREDEFINED_FORMS[form]
        else:
            # Forma libre: distribuir compases equitativamente
            n = len(self.sections_labels)
            frac = 1.0 / n
            self._form_template = [(lbl, frac, None)
                                   for lbl in self.sections_labels]

    def _key_for_section(self, section_label: str) -> Tuple[int, str]:
        """Devuelve (root_pc, mode) para una sección dada."""
        base_label = section_label.rstrip("'")

        # Sección A y sus variantes siempre en la tonalidad base
        if base_label == "A":
            return self.root_pc, self.mode

        # Secciones con función de contraste secundario (B, V, verse)
        secondary = {"B", "V", "T", "verse"}
        # Secciones con función de contraste máximo (C, D, chorus, development)
        tertiary  = {"C", "D", "R", "E", "chorus", "development", "bridge"}

        if base_label in secondary:
            key_scheme = self.key_b
        elif base_label in tertiary:
            key_scheme = self.key_c
        else:
            # Etiquetas desconocidas en tonalidad base
            return self.root_pc, self.mode

        shift = self.KEY_SHIFT.get(key_scheme, 0)

        if key_scheme == "parallel":
            new_mode = ("major" if self.mode == "minor" else "minor")
            return self.root_pc, new_mode

        new_root = (self.root_pc + shift) % 12
        return new_root, self.mode

    def _profile_for_section(self, section_label: str) -> str:
        """Devuelve el perfil emocional para una sección."""
        base_label = section_label.rstrip("'")
        if base_label == "A":
            return self.base_profile
        shift_map = SECTION_PROFILE_SHIFT.get(base_label, {})
        return shift_map.get(self.base_profile, self.base_profile)

    def _chords_for_section(self, section_label: str,
                             root_pc: int, mode: str,
                             bars: int) -> str:
        """Devuelve la progresión de acordes para la sección."""
        base_label = section_label.rstrip("'")
        is_prima   = section_label.endswith("'")

        # Si el usuario proporcionó una progresión específica
        if base_label == "A" and self.chords_a:
            prog = self.chords_a
            if is_prima:
                prog = _vary_progression(prog, self.rng, root_pc, mode)
            return prog

        if base_label == "B" and self.chords_b:
            shift = self.KEY_SHIFT.get(self.key_b, 0)
            return _transpose_chord_progression(
                self.chords_b, shift, root_pc, mode)

        if base_label == "C" and self.chords_c:
            shift = self.KEY_SHIFT.get(self.key_c, 0)
            return _transpose_chord_progression(
                self.chords_c, shift, root_pc, mode)

        # Generar progresión automática según función de la sección
        scale_ivs = SCALE_INTERVALS.get(mode, SCALE_INTERVALS["major"])
        root_name = NOTE_NAMES[root_pc]

        def degree(n: int, q: str = "") -> str:
            if n < len(scale_ivs):
                pc = (root_pc + scale_ivs[n]) % 12
                return NOTE_NAMES[pc] + q
            return root_name

        # Progresiones características por sección
        beats_per_bar_int = int(self.time_sig.split("/")[0])
        b = beats_per_bar_int

        if base_label in ("A", "V"):
            # I - V - VI - IV  (la progresión más universal)
            q6 = "m" if mode in ("major", "mixolydian", "lydian") else ""
            prog = (f"{degree(0)}:{b} {degree(4)}:{b} "
                    f"{degree(5)}{q6}:{b} {degree(3)}:{b}")

        elif base_label in ("B", "T"):
            # VI - IV - I - V  (relación tónica-subdominante más lírica)
            q6 = "m" if mode in ("major", "mixolydian") else ""
            prog = (f"{degree(5)}{q6}:{b} {degree(3)}:{b} "
                    f"{degree(0)}:{b} {degree(4)}:{b}")

        elif base_label in ("C", "D"):
            # II - V - I  (progresión cadencial, más tensión)
            q2 = "m7" if mode in ("major", "lydian", "mixolydian") else "7"
            prog = (f"{degree(1)}{q2}:{b} {degree(4)}7:{b*2} "
                    f"{degree(0)}:{b}")

        elif base_label == "E":
            # I - IV - V  (exposición simple)
            prog = (f"{degree(0)}:{b*2} {degree(3)}:{b} {degree(4)}:{b}")

        elif base_label == "R":
            # I - VI - II - V  (recapitulación con pedal)
            q6 = "m" if mode in ("major",) else ""
            q2 = "m" if mode in ("major",) else ""
            prog = (f"{degree(0)}:{b} {degree(5)}{q6}:{b} "
                    f"{degree(1)}{q2}:{b} {degree(4)}:{b}")

        else:
            # Fallback: I - IV - V - I
            prog = (f"{degree(0)}:{b} {degree(3)}:{b} "
                    f"{degree(4)}:{b} {degree(0)}:{b}")
    ...
    def _contour_for_section(self, section_label: str,
                              profile: str) -> str:
        """Contorno para una sección."""
        base_label = section_label.rstrip("'")
        if base_label == "A":
            return self.contour_a if self.contour_a != "auto" else "arch"
        if base_label == "B":
            return self.contour_b if self.contour_b != "auto" else "wave"
        return PROFILE_CONTOUR.get(profile, "arch")

    def _engine_for_section(self, section_label: str) -> str:
        """Motor para una sección — puede variar si es sección B con interpolación."""
        base = self.base_engine
        # En sección C podemos usar tension_arc para mayor dramatismo
        if section_label.rstrip("'") in ("C", "D") and base == "chord_guided":
            return "tension_arc"
        return base

    def _build_bridge(self, from_key: int, to_key: int,
                       bars: int, tempo: int,
                       time_sig: str) -> Optional[MelodyResult]:
        """Genera un puente de modulación entre dos tonalidades."""
        if bars <= 0 or from_key == to_key:
            return None

        from_name = NOTE_NAMES[from_key]
        to_name   = NOTE_NAMES[to_key]
        bridge_chords = _modulate_progression(
            f"{from_name}:4", from_key, to_key, self.mode)
        ...
        gen = ConditionedMelodyGenerator(
            ...
            engine        = "tension_arc",
            profile       = "tense",
            rhythm        = self.base_rhythm,
            contour       = "ascending",
            pitch_low     = self.pitch_low,
            pitch_high    = self.pitch_high,
            chord_events  = parse_chords_text(
                bridge_chords, from_key, self.mode),
            seed          = self.seed + 9999,
            verbose       = False,
        )
        return gen.generate_once()

    def compose(self) -> FormResult:
        """
        Genera la obra completa sección a sección.

        Para cada sección:
          1. Determina tonalidad, perfil, motor, progresión y contorno.
          2. Si es prima (A'), aplica variación a la progresión.
          3. Genera el mejor candidato de la sección.
          4. Si hay cambio de tonalidad y bridge_bars > 0,
             inserta un puente modulante.
          5. Ajusta los offsets de las notas para la posición en la obra.

        Devuelve un FormResult con todas las secciones y sus resultados.
        """
        # Calcular compases por sección
        template = self._form_template
        # Ajustar fracciones a labels presentes en self.sections_labels
        labels_in_template = [t[0] for t in template]

        section_bars: List[int] = []
        remaining = self.total_bars
        for i, (lbl, frac, _) in enumerate(template):
            if i == len(template) - 1:
                bars = remaining
            else:
                bars = max(2, int(round(self.total_bars * frac / 2) * 2))
                bars = min(bars, remaining - (len(template) - i - 1) * 2)
            section_bars.append(bars)
            remaining -= bars

        if self.verbose:
            print(f"\n[form] Forma: {self.form_string}")
            for (lbl, _, _), bars in zip(template, section_bars):
                print(f"  {lbl}: {bars} compases")

        # Caché de resultados por label base (para reutilizar A en repeticiones)
        results_cache: Dict[str, MelodyResult] = {}
        sections: List[Tuple[str, MelodyResult]] = []
        global_offset_beats = 0.0
        beats_per_bar = float(self.time_sig.split("/")[0])
        prev_root_pc  = self.root_pc

        for (lbl, _, _), bars in zip(template, section_bars):
            base_label = lbl.rstrip("'")
            is_prima   = lbl.endswith("'")
            root_pc, mode = self._key_for_section(lbl)
            profile  = self._profile_for_section(lbl)
            engine   = self._engine_for_section(lbl)
            chords   = self._chords_for_section(lbl, root_pc, mode, bars)
            contour  = self._contour_for_section(lbl, profile)

            if self.verbose:
                print(f"\n[form] Sección {lbl}  bars={bars}  "
                      f"key={NOTE_NAMES[root_pc]}  profile={profile}  "
                      f"engine={engine}")
                print(f"       chords: {chords}")

            # Puente modulante si cambia la tonalidad
            if self.bridge_bars > 0 and prev_root_pc != root_pc:
                bridge = self._build_bridge(
                    prev_root_pc, root_pc, self.bridge_bars,
                    self.tempo, self.time_sig)
                if bridge:
                    # Ajustar offsets del puente
                    bridge_notes = [
                        MelodyNote(n.pitch, n.duration, n.velocity,
                                   round(n.offset + global_offset_beats, 4))
                        for n in bridge.notes
                    ]
                    bridge_result = MelodyResult(
                        notes=bridge_notes,
                        key=NOTE_NAMES[prev_root_pc],
                        mode=self.mode,
                        bars=self.bridge_bars,
                        tempo=self.tempo,
                        engine="tension_arc",
                        profile="tense",
                        score=bridge.score,
                        score_detail=bridge.score_detail,
                        metadata={"section": "bridge"},
                    )
                    sections.append((f"bridge-{lbl}", bridge_result))
                    global_offset_beats += self.bridge_bars * beats_per_bar

            # Reutilizar resultado cacheado para repeticiones exactas (sin prima)
            if not is_prima and base_label in results_cache:
                cached = results_cache[base_label]
                # Reutilizar notas pero con offset ajustado a la posición actual
                section_notes = [
                    MelodyNote(n.pitch, n.duration, n.velocity,
                               round(n.offset + global_offset_beats, 4))
                ...
                result = MelodyResult(
                    notes=section_notes,
                    key=cached.key, mode=cached.mode,
                    bars=bars, tempo=self.tempo,
                    engine=cached.engine, profile=cached.profile,
                    score=cached.score, score_detail=cached.score_detail,
                    metadata={**cached.metadata, "section": lbl, "reused": True},
                )
            else:
                # Parsear acordes con la tonalidad correcta
                chord_events = parse_chords_text(chords, root_pc, mode)

                gen = ConditionedMelodyGenerator(
                    key           = NOTE_NAMES[root_pc],
                    mode          = mode,
                    bars          = bars,
                    tempo         = self.tempo,
                    ...
                    ornament_types = self.ornament_types,
                    ornament_prob  = self.ornament_prob,
                    fix_avoid     = self.fix_avoid,
                    lstm_model    = self.lstm_model,
                    motif_notes   = self.motif_notes,
                    interp_profile_b = self._profile_for_section("B"),
                    interp_alpha  = self.interp_alpha,
                    seed          = self.seed + hash(lbl) % 1000,
                    verbose       = False,
                )

                # Generar 3 candidatos y elegir el mejor
                candidates = gen.generate_candidates(3)
                best = candidates[0]

                # Ajustar offsets a la posición global en la obra
                section_notes = [
                    MelodyNote(n.pitch, n.duration, n.velocity,
                               round(n.offset + global_offset_beats, 4))
                    for n in best.notes
                ...
                result = MelodyResult(
                    ...
                    bars=bars, tempo=self.tempo,
                    engine=best.engine, profile=best.profile,
                    score=best.score, score_detail=best.score_detail,
                    metadata={**best.metadata, "section": lbl},
                )

                # Cachear la primera aparición de cada label base
                if base_label not in results_cache:
                    results_cache[base_label] = MelodyResult(
                        notes=best.notes,  # offsets locales
                        key=best.key, mode=best.mode,
                        bars=bars, tempo=self.tempo,
                        engine=best.engine, profile=best.profile,
                        score=best.score, score_detail=best.score_detail,
            ...
            sections.append((lbl, result))
            global_offset_beats += bars * beats_per_bar
            prev_root_pc = root_pc

            if self.verbose:
                print(f"  → score={result.score:.3f}  notas={len(result.notes)}")
    ...
    def export(self, form_result: FormResult,
               output_path: str,
               include_section_markers: bool = True,
               include_chords: bool = False) -> str:
        """
        Exporta la obra completa a un único MIDI.

        Si include_section_markers=True, añade texto de marcador
        (MetaMessage 'marker') al inicio de cada sección — visible
        en la mayoría de DAWs como marcadores de edición.
        """
        if not MIDO_OK:
            print("[ERROR] mido requerido.")
            return ""

        tpb   = 480
        mid   = mido.MidiFile(ticks_per_beat=tpb)
        tempo = int(60_000_000 / max(self.tempo, 1))
        bpb   = float(self.time_sig.split("/")[0])

        def q_ticks(q: float) -> int:
            return int(q * tpb)

        # Track 0: metadatos + marcadores de sección
        t0 = mido.MidiTrack()
        mid.tracks.append(t0)
        t0.append(mido.MetaMessage("set_tempo", tempo=tempo, time=0))
        num, den = self.time_sig.split("/")
        t0.append(mido.MetaMessage("time_signature",
                                    numerator=int(num),
                                    denominator=int(den),
                                    clocks_per_click=24,
        ...
        if include_section_markers:
            # Los marcadores van en el track 0 con tiempo absoluto→delta
            marker_events = []
            for lbl, res in form_result.sections:
                if res.notes:
                    abs_tick = q_ticks(res.notes[0].offset)
                    marker_events.append((abs_tick, lbl))
            marker_events.sort(key=lambda e: e[0])
            cur_tick = 0
            for abs_t, lbl in marker_events:
                delta = max(0, abs_t - cur_tick)
                t0.append(mido.MetaMessage("marker",
                                            text=f"Section {lbl}",
                                            time=delta))
                cur_tick = abs_t

        # Track 1: melodía completa
        t1 = mido.MidiTrack()
        mid.tracks.append(t1)
        t1.append(mido.MetaMessage("track_name", name="Melody", time=0))
        t1.append(mido.Message("program_change", channel=0, program=73, time=0))

        # Recoger todas las notas de todas las secciones
        all_notes = []
        for _, res in form_result.sections:
            all_notes.extend(res.notes)
        ...
        for n in all_notes:
            if n.pitch < 0:
                continue
            events.append((q_ticks(n.offset),            "note_on",  n.pitch, n.velocity))
            events.append((q_ticks(n.offset + n.duration), "note_off", n.pitch, 0))
        events.sort(key=lambda e: (e[0], 0 if e[1] == "note_off" else 1))

        cur = 0
        for abs_t, mtype, note, vel in events:
            delta = max(0, abs_t - cur)
            t1.append(mido.Message(mtype, channel=0, note=note,
                                   velocity=vel, time=delta))
            cur = abs_t

        # Track 2 (opcional): acordes ensamblados de todas las secciones
        if include_chords:
            t2 = mido.MidiTrack()
            mid.tracks.append(t2)
            t2.append(mido.MetaMessage("track_name", name="Chords", time=0))
            t2.append(mido.Message("program_change", channel=1, program=48, time=0))

            chord_evts = []
            for lbl, res in form_result.sections:
                # Reconstruir los chord_events para esta sección con offset global
                root_pc, mode = self._key_for_section(lbl)
                chords_text   = self._chords_for_section(lbl, root_pc, mode, res.bars)
                c_evs = parse_chords_text(chords_text, root_pc, mode)

                # Calcular offset global de la sección
                if res.notes:
                    sec_offset = res.notes[0].offset
                else:
                    continue

                for ev in c_evs:
                    pitches = [ev.root_pc + 48] + [
                        ev.root_pc + 48 + iv
                        for iv in CHORD_INTERVALS.get(ev.quality, [0, 4, 7])
                    ]
                    on_t  = q_ticks(sec_offset + ev.offset)
                    off_t = q_ticks(sec_offset + ev.offset + ev.duration)
                    for p in pitches:
                        if 0 <= p <= 127:
                            chord_evts.append((on_t,  "note_on",  p, 55))
                            chord_evts.append((off_t, "note_off", p, 0))
            ...
            for abs_t, mtype, note, vel in chord_evts:
                delta = max(0, abs_t - cur)
                t2.append(mido.Message(mtype, channel=1, note=note,
                                       velocity=vel, time=delta))
                cur = abs_t
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--form", type=str, default=None,
                   help=("Estructura formal. Formas predefinidas: "
                         "ABA | ABAB | AABA | ABACABA | verse_chorus | sonata. "
                         "O libre: \"A B A' C A\" o \"ABACA\""))
    p.add_argument("--form-total-bars", type=int, default=64,
                   help="Total de compases de la obra (default: 64)")
    p.add_argument("--form-engine", type=str, default=None,
                   help="Motor base para todas las secciones (default: --engine)")
    p.add_argument("--form-chords-a", type=str, default=None,
                   help="Progresión de acordes para sección A")
    p.add_argument("--form-chords-b", type=str, default=None,
                   help="Progresión de acordes para sección B")
    p.add_argument("--form-chords-c", type=str, default=None,
                   help="Progresión de acordes para sección C")
    p.add_argument("--form-key-b", type=str, default="same",
                   choices=["same", "relative", "dominant", "subdominant",
                            "parallel", "mediant", "submediant"],
                   help="Tonalidad de sección B respecto a A (default: same)")
    p.add_argument("--form-key-c", type=str, default="relative",
                   choices=["same", "relative", "dominant", "subdominant",
                            "parallel", "mediant", "submediant"],
                   help="Tonalidad de sección C respecto a A (default: relative)")
    p.add_argument("--form-bridge-bars", type=int, default=0,
                   help="Compases de puente entre secciones con distinta tonalidad (default: 0)")
    p.add_argument("--form-contour-a", type=str, default="auto",
                   help="Contorno para sección A (default: auto)")
    p.add_argument("--form-contour-b", type=str, default="auto",
                   help="Contorno para sección B (default: auto)")
    p.add_argument("--form-markers", action="store_true",
                   help="Incluir marcadores de sección en el MIDI (default: True)",
                   default=True)
...
def main():
    ...
    # ── Modo FORM ────────────────────────────────────────────────────────────
    if args.form:
        base_engine = args.form_engine or args.engine
        print(f"\n[form] Componiendo forma '{args.form}'  "
              f"total={args.form_total_bars} compases  "
              f"engine={base_engine}  profile={args.profile}")

        composer = FormComposer(
            key          = args.key,
            mode         = args.mode,
            total_bars   = args.form_total_bars,
            tempo        = args.tempo,
            time_sig     = args.time_sig,
            form         = args.form,
            base_engine  = base_engine,
            base_profile = args.profile,
            base_rhythm  = args.rhythm,
            chords_a     = args.form_chords_a or args.chords,
            chords_b     = args.form_chords_b,
            chords_c     = args.form_chords_c,
            key_b        = args.form_key_b,
            key_c        = args.form_key_c,
            bridge_bars  = args.form_bridge_bars,
            contour_a    = args.form_contour_a,
            contour_b    = args.form_contour_b,
        ...
        form_result = composer.compose()

        # Exportar
        base_out = args.output.replace(".mid", "")
        out_path = f"{base_out}.mid"
        composer.export(
            form_result,
            output_path             = out_path,
            include_section_markers = args.form_markers,
            include_chords          = args.include_chords,
        )

        # JSON con el mapa de secciones
        json_path = f"{base_out}.form.json"
        form_json = {
            "form":       form_result.form_string,
            "key":        form_result.key,
            "tempo":      form_result.tempo,
            "total_bars": form_result.total_bars,
            "total_notes": form_result.total_notes,
            "sections": [
                {
                    "label":   lbl,
                    "bars":    res.bars,
                    ...
                    "notes":   len(res.notes),
                    "offset_beats": round(res.notes[0].offset, 2) if res.notes else 0,
                }
                for lbl, res in form_result.sections
            ],
        }
        ...
        print(f"\n╔═ FORMA COMPLETADA ══════════════════════════════════╗")
        print(f"║  Forma     : {form_result.form_string}")
        print(f"║  Tonalidad : {form_result.key}")
        print(f"║  Compases  : {form_result.total_bars}  Notas: {form_result.total_notes}")
        for lbl, res in form_result.sections:
            reused = res.metadata.get("reused", False)
            tag    = " [rep]" if reused else ""
            print(f"║  {lbl:4s}  {res.bars:3d} bars  "
                  f"{res.key:3s} {res.mode:<14s}  "
                  f"{res.profile:<12s}  score={res.score:.3f}{tag}")
        print(f"╚══════════════════════════════════════════════════════╝")
