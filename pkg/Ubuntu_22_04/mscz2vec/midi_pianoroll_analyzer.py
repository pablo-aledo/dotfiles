...
def build_bar_vectors(tracks: List[Dict], n_bars: int) -> List[List[float]]:
    """Build a pitch-class + density feature vector per bar across all tracks."""
    vecs = []
    for bar_i in range(n_bars):
        ...
        for t in tracks:
            for n in t['notes']:
                if n['start_bar'] == bar_i:
                    pc_hist[n['pitch'] % 12] += 1
                    n_notes += 1
        total = sum(pc_hist) or 1
        vec = [x / total for x in pc_hist] + [min(1.0, n_notes / 10.0)]
        vecs.append(vec)
    return vecs
...
def assign_section_labels(boundaries: List[int], n_bars: int,
    ...
    """
    Assign section labels A, B, C… by clustering boundary segments.
    Similar segments get the same letter.
    """
    segments = []
    prev = 0
    for b in boundaries + [n_bars]:
        if b > prev:
            ...
            centroid = [sum(x[i] for x in bar_vecs)/len(bar_vecs) for i in range(len(bar_vecs[0]))]
            segments.append((prev, b, centroid))
        prev = b
    ...
    for si, (start, end, cent) in enumerate(segments):
        best_match = -1
        best_sim   = 0.96  # high threshold to distinguish sections
        for j, (_, _, prev_cent) in enumerate(centroids_seen):
            s = cosine_sim(cent, prev_cent)
            if s > best_sim:
                best_sim  = s
                best_match = j
        if best_match >= 0:
            labels[si] = labels[best_match]
        else:
            labels[si] = chr(ord('A') + letter_counter)
            letter_counter += 1
        centroids_seen.append((start, end, cent))
...
def compute_global_sections(tracks: List[Dict], n_bars: int) -> Dict:
    if n_bars < 2:
        return {'bar_labels': ['A'] * n_bars, 'boundaries': [], 'n_bars': n_bars}
    vecs = build_bar_vectors(tracks, n_bars)
    ssm  = ssm_matrix(vecs)
    k    = min(4, max(2, n_bars // 8))
    nov  = novelty_curve(ssm, kernel_size=k)
    # Dynamic threshold: use mean + 0.3 of normalized curve
    mean_nov = sum(nov) / len(nov) if nov else 0
    threshold = max(0.35, mean_nov + 0.15)
    min_gap   = max(2, n_bars // 10)
...
def analyze(mid_path: str, cfg: dict) -> str:
    ...
    sections = compute_global_sections(tracks, n_bars)
