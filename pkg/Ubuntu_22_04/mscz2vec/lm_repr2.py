...
def roll_to_midi(roll: np.ndarray,
    ...
    Algoritmo:
    - Para cada familia, para cada pitch: detecta runs de frames activos.
    - Cada run = una nota con duración = len(run) * frame_ms.
    - La velocity es el máximo del run normalizado de vuelta a [0,127].

    instruments: lista de dicts {'name', 'role'} compatible con orchestrator.py.
                 Un instrumento por familia — si hay varios en la misma familia
                 se usa el primero.
    ...
    for fam_idx, fam in enumerate(FAMILIES):
        ...
        for pi in range(N_PITCHES):
            ...
            # Detectar runs de frames activos para este pitch
            in_note  = False
            run_start = 0
            run_vels  = []
            ...
            for f in range(T):
                ...
                if active and not in_note:
                    ...
                    # Fin del run → generar nota
                    vel_midi = int(np.clip(max(run_vels) * 127, 1, 127))
                    tick_on  = run_start * frame_ticks
                    tick_off = f * frame_ticks
                    note_events.append((tick_on,  'on',  pitch, vel_midi))
                    note_events.append((tick_off, 'off', pitch, 0))
                    in_note  = False
