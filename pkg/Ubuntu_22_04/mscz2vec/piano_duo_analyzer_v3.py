...
def detect_sections(chords_per_bar: List[Dict], density: List[float],
                    n_bars: int, bpb: float,
                    motifs: List[Dict] = None) -> List[Dict]:
    """
    Section detection using motif recurrence as primary signal.

    If motifs are provided, section boundaries are placed at the start of
    each new motif occurrence (when the piece 'restarts' a theme).
    Sections must be at least n_bars//6 bars long.

    Fallback: if no motifs or too few boundaries, divide into equal thirds/quarters.
    """
    if n_bars < 4:
        return [{'bar': 0, 'end': n_bars, 'label': 'A', 'color': '#3b82f6'}]

    MIN_SECTION = max(6, n_bars // 6)
    ...
    # Primary: motif recurrences as section markers
    if motifs:
        for m in motifs:
            for occ in m['occurrences']:
                bar = int(occ['start_beat'] / bpb)
    ...
    # If still only 1 section for a long piece, divide equally
    if len(boundaries) == 1 and n_bars >= MIN_SECTION * 2:
        n_divs = max(2, min(4, n_bars // MIN_SECTION))
        step = n_bars // n_divs
        boundaries = [i * step for i in range(n_divs)]

    sections = []
    for i, bar in enumerate(boundaries):
        end = boundaries[i+1] if i+1 < len(boundaries) else n_bars
        sections.append({'bar': bar, 'end': end,
...
def analyze(mid_path: str, right_idx: int, left_idx: int,
    ...
    # (whichever track is non-harmony in each bar)
    melody_notes = (
    ...
    sections  = detect_sections(chords_per_bar, density, n_bars, bpb, motifs=motifs)
    print(f"  {len(motifs)} motivos, {len(cadences)} cadencias, {len(sections)} secciones")
