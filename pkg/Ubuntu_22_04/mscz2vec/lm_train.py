"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  LM_TRAIN  v1.0  —  Loop de entrenamiento del VAE latente                  ║
║                                                                              ║
║  Entrena MultiInstrumentVAE sobre tu corpus de MIDIs.                       ║
...
║    python lm_train.py --midi-dir ./midis --resume ./checkpoints/last.pt    ║
║                                                                              ║
║    # Solo validar un checkpoint existente                                   ║
║    python lm_train.py --midi-dir ./midis --eval-only ./checkpoints/best.pt ║
║                                                                              ║
║  SALIDAS:                                                                    ║
║    checkpoints/best.pt      — mejor modelo según val loss                  ║
...
try:
    from lm_repr    import TENSOR_DIMS, FAMILIES
    from lm_dataset import (MidiLatentDataset, get_dataloader,
                             build_cache, load_cache)
    from lm_model   import (MultiInstrumentVAE, VAELoss,
                             DEFAULT_CFG, save_model, load_model)
...
class TrainingLogger:
    ...
    def plot(self, out_path: str):
        ...
        fig.suptitle('Curvas de entrenamiento — MultiInstrumentVAE', fontsize=13)
...
def train_epoch(model: MultiInstrumentVAE,
                loader,
...
@torch.no_grad()
def val_epoch(model: MultiInstrumentVAE,
              loader,
              loss_fn: VAELoss) -> dict:
...
def train(args):
    ...
    print("\n" + "═"*65)
    print("  LM_TRAIN — MultiInstrumentVAE")
    print("═"*65)
    ...
    aug_cfg = {
        ...
        'family_dropout_p': args.dropout_p,
        'noise_sigma':      args.noise_sigma,
        'time_reverse_p':   args.reverse_p,
    ...
    loader_train = get_dataloader(ds_train, batch_size=args.batch_size, shuffle=True)
    loader_val   = get_dataloader(ds_val,   batch_size=args.batch_size, shuffle=False)
    ...
    cfg = {
        **DEFAULT_CFG,          # incluye kl_free_bits=1.0, w_density=5.0
    ...
    if args.resume:
        ...
        model, extra = load_model(args.resume)
    ...
    if args.resume:
        try:
            _, extra = load_model(args.resume)
    ...
    for i, batch in enumerate(loader_train):
        if i >= 3:
            break
        x_b, bpm_b = batch['tensor'], batch['bpm']
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_model(model, last_path, extra)

        if is_best:
            save_model(model, best_path, extra)

        if args.save_every > 0 and (epoch + 1) % args.save_every == 0:
            snap_path = os.path.join(args.checkpoint_dir, f'epoch{epoch+1:04d}.pt')
            save_model(model, snap_path, extra)
...
def eval_only(args):
    ...
    model, extra = load_model(args.eval_only)
    ...
    from lm_repr import DIMS_PER_FAMILY, IDX_DENSITY
    print(f"  {'Familia':<12}  {'Original':>10}  {'Recon':>10}")
    for fi, fam in enumerate(FAMILIES):
        base = fi * DIMS_PER_FAMILY
        orig_act = float(x_np[:, base + IDX_DENSITY].mean())
        rec_act  = float(rec_np[:, base + IDX_DENSITY].mean())
