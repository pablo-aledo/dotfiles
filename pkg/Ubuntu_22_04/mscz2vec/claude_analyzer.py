...
def generate_report(
    ...
    # ── CABECERA ───────────────────────────────────────────────
    L += [SEP, f"  🎵  ANÁLISIS MUSICAL COMPLETO  v8.0",
          f"  {title.upper()}", SEP]
    ...
    for trope_name, trope_data in (ci.get('tropes') or {}).items():
        ...
        L.append(f"    {trope_data.get('desc','')}")
        if trope_data.get('times'):
            L.append(f"    Apariciones: {', '.join(trope_data['times'][:3])}")
    ...
    for c2 in co.get('contradictions',[]):
        L.append(f"  ↔ [{c2['type']}] {c2['desc']}")

    # ══ 42. HISTORIA EMOCIONAL ═══════════════════════════════
    L += ["", f"╔══ 42. HISTORIA EMOCIONAL (arco de personaje) {'═'*17}", ""]
    if story:
    ...
    L.append(f"  {cat.get('desc','')}")
    if cat.get('present'):
        L.append(f"  Acumulación desde : {ts_str(cat.get('buildup_start') or 0)}  "
                 f"(duración {cat.get('buildup_duration',0):.0f}s)")
        L.append(f"  Momento liberación: {ts_str(cat.get('moment') or 0)}")
        L.append(f"  Fuerza liberación : {cat.get('release_strength',0):.2f}/1.0")
        L.append(f"  Calidad resolución: {cat.get('resolution_quality',0):.2f}/1.0")
        if len(cat.get('waves', [])) > 1:
            L.append(f"  Oleadas totales   : {len(cat['waves'])}")
            for i, w in enumerate(cat['waves'][:4]):
                L.append(f"    [{i+1}] acum {ts_str(w['buildup_start'])} → pico {ts_str(w['peak_time'])} → liberación {ts_str(w['release_time'])}  (fuerza={w['release_strength']:.2f})")
    L.append(f"")
    L.append(f"  Efecto sobre el oyente:")
    for line in wrap(cat.get('emotional_effect',''), width=64, indent='    ').split('\n'):
        L.append(line)

    # ══ 74. MAPA DE AMBIVALENCIA EMOCIONAL ══════════════════
    amb = emotional_ambivalence or {}
    L += ["", "╔══ 74. MAPA DE AMBIVALENCIA EMOCIONAL " + "═"*26]
    L.append(f"  {amb.get('desc','')}")
    L.append(f"  Índice medio   : {amb.get('mean_ambivalence',0):.3f}/1.0")
    L.append(f"  Perfil         : {amb.get('profile','')}")
    if amb.get('peak_moments'):
        L.append(f"  Momentos de máxima ambivalencia:")
        for pm in amb['peak_moments'][:4]:
            L.append(f"    ⊗ {ts_str(pm['time'])}  [{pm['ambivalence']:.2f}]  {pm['desc']}")
    if amb.get('zones'):
        L.append(f"  Zonas de conflicto emocional activo ({len(amb['zones'])} zona(s)):")
        for z in amb['zones'][:4]:
            L.append(f"    {ts_str(z['start'])} → {ts_str(z['end'])}  ({z['duration']:.1f}s)")

    # ══ 75. TRAYECTORIA EMOCIONAL VA ════════════════════════
    traj = emotional_trajectory or {}
    L += ["", "╔══ 75. TRAYECTORIA EN ESPACIO VALENCE-AROUSAL " + "═"*17]
    L.append(f"  {traj.get('desc','')}")
    L.append(f"  Distancia total : {traj.get('total_distance',0):.3f} unidades VA")
    L.append(f"  Desplaz. neto   : {traj.get('net_displacement',0):.3f}  |  "
             f"Eficiencia: {traj.get('efficiency',0):.2f}")
    L.append(f"  Forma camino    : {traj.get('path_shape','?')} — {traj.get('path_shape_desc','')}")
    cx, cy = traj.get('centroid', (0.0, 0.0))
    L.append(f"  Centroide VA    : ({cx:+.2f}, {cy:+.2f})")
    L.append(f"  Cruces de eje   : {traj.get('crossings',0)}")
    L.append(f"  Cuadrante dom.  : {traj.get('dominant_quadrant','?')}")
    qt = traj.get('quadrant_time', {})
    if qt:
        L.append(f"  Tiempo por cuadrante:")
    ...
    L.append(f"  Perfil de voz      : {ac.get('voice_profile','')}")
    if ac.get('signature_choices'):
        L.append(f"  Firma compositiva  : {' | '.join(ac['signature_choices'][:3])}")
    devs = ac.get('deviations', [])
    if devs:
        L.append(f"  Desviaciones detectadas ({len(devs)}):")
        for d in devs[:5]:
            L.append(f"  ▸ [{d['type']}]  peso={d['weight']:.2f}")
            for line in wrap(d['desc'], width=62, indent='      ').split('\n'):
                L.append(line)
    else:
        L.append("  Sin desviaciones significativas — escritura convencional para su contexto.")

    # ══ 77. DENSIDAD INFORMACIONAL MULTINIVEL ═══════════════
    mld = multilevel_density or {}
    L += ["", "╔══ 77. DENSIDAD INFORMACIONAL MULTINIVEL " + "═"*23]
    L.append(f"  {mld.get('desc','')}")
    L.append(f"  Entropía micro  : {mld.get('micro_entropy',0):.3f} bits  (escala nota)")
    L.append(f"  Entropía meso   : {mld.get('meso_entropy',0):.3f} bits  (escala frase)")
    L.append(f"  Entropía macro  : {mld.get('macro_entropy',0):.3f} bits  (escala sección)")
    L.append(f"  Contraste niveles: {mld.get('level_contrast',0):.3f}")
    L.append(f"  Economía comp.  : {mld.get('composer_economy',0):.2f}/1.0")
    L.append(f"  Perfil          : {mld.get('density_profile','?')} — {mld.get('profile_desc','')}")
    nc = mld.get('novelty_curve', [])
    if nc:
        L.append(f"  Novedad por sección:")
        for item in nc:
            bar_w = int(item.get('novelty', 0) * 20)
            bar = '█' * bar_w + '░' * (20 - bar_w)
            L.append(f"    Sec.{item['section']}  [{bar}]  novelty={item.get('novelty',0):.2f}  H={item.get('entropy',0):.2f}b")

    # ══ 78. INTENCIÓN NARRATIVA DEL COMPOSITOR ══════════════
    ni = narrative_intention or {}
    L += ["", "╔══ 78. INTENCIÓN NARRATIVA DEL COMPOSITOR " + "═"*21]
    L.append(f"  Arquetipo narrativo : {ni.get('archetype','?')}  "
             f"(confianza: {ni.get('confidence',0):.0%})")
    if ni.get('alternative'):
        L.append(f"  Arquetipo alternativo: {ni['alternative']}")
    L.append(f"  Relación con el tiempo:")
    for line in wrap(ni.get('time_relationship',''), width=64, indent='    ').split('\n'):
        L.append(line)
    L.append(f"  Estrategia de expectativa:")
    for line in wrap(ni.get('expectation_strategy',''), width=64, indent='    ').split('\n'):
        L.append(line)
    L.append(f"  Intención inferida:")
    for line in wrap(ni.get('composer_intent',''), width=64, indent='    ').split('\n'):
        L.append(line)
    if ni.get('evidence'):
        L.append(f"  Evidencias:")
        for ev in ni['evidence'][:4]:
            L.append(f"    ✓ {ev}")
    ...
    L.append(f"  Concepto más próximo : '{se.get('concept','?')}'  "
             f"(ajuste={se.get('fit_score',0):.2f}/1.0)")
    L.append(f"  Justificación teórica:")
    for line in wrap(se.get('justification',''), width=62, indent='    ').split('\n'):
        L.append(line)
    L.append(f"  Arco narrativo       : {se.get('arc_desc','')}")
    L.append(f"  Persona afín         : {se.get('persona_name','?')} — {se.get('persona_desc','')}")
    L.append(f"  Lente de análisis:")
    for line in wrap(se.get('persona_lens',''), width=62, indent='    ').split('\n'):
        L.append(line)
    alts = se.get('alternatives', [])
    if alts:
        L.append(f"  Conceptos alternativos: "
    ...
    # ══ 71. ENSAYO MUSICAL ESTRUCTURADO ═══════════════════════
    if essay_text:
    ...
    # ══ 28 (renumber) → 43. SÍNTESIS TÉCNICA ════════════════
    # ══ 28. SÍNTESIS NARRATIVA ═══════════════════════════════
    # ══ 16. SÍNTESIS NARRATIVA ═══════════════════════════════
...
def write_harmony(chords, key_root, mode, canon_progs, modal_borrow,
                  ...
                  voice_leading, pedal_points, elliptical) -> str:
    lines = []
    lines.append("─" * 66)
...
def write_rhythm(rhythm, micro_timing, metric_hierarchy, polyrhythm,
                 groove, bass_narrative, layer_sync, bpm, time_sig) -> str:
    lines = []
    lines.append("─" * 66)
...
def write_listener_experience(listener_model, cumulative_state, auditory_fatigue,
    ...
    lm_mean = lm.get('mean_experience', 0)
    ...
    cs_stress = cs.get('max_cumulative_stress', 0)
    fv, fa = cs.get('final_state', (0, 0))
    if cs_journey:
    ...
    # Suspense
    sp = suspense or {}
    sp_idx = sp.get('suspense_index', 0)
...
def write_compositional_intention(fractal, emotional_weight, golden,
    ...
    # Emotional coherence / contradictions
...
def write_synthesis(key_name, mode, avg_bpm, total_dur, emotional_genre,
    ...
    t_word, _ = tempo_desc(avg_bpm)

    # Opening
    dur_min = int(total_dur//60); dur_sec = int(total_dur%60)
    p1 = (f"  Esta pieza en {key_name} — {dur_min}m{dur_sec:02d}s a "
          f"{avg_bpm:.0f} BPM en modo {mode_label} — "
          f"pertenece al género emocional '{top_genre}' "
          f"con un {top_score*100:.0f}% de afinidad")
    if len(ranking) > 1:
        p1 += f", seguido de '{ranking[1][0]}' ({ranking[1][1]*100:.0f}%)"
    p1 += "."
    lines.append(wrap_prose(p1, indent='  '))
    lines.append("")
    ...
    closing = (f"  En definitiva: una pieza de {mode_core}, "
               f"construida con rigor técnico y orientada "
               f"hacia {'la introspección y la escucha íntima' if avg_bpm < 90 else 'la narrativa emocional densa'}. "
               f"El análisis confirma lo que el oído percibe: "
               f"{'no hay resolución porque no se busca — la tensión es el estado' if sa_ratio < 0.05 and (not pn_time or pn_pos < 0.85) else 'la pieza sabe adónde va y llega'}.")
...
def generate_essay(filepath, **kw) -> str:
    ...
    parts.append(f"  Ensayo estructurado · midi_analyzer v7.0")
    parts.append(SEP)
    parts.append("")
...
def run_analysis(filepath: str, n_sections: int = 6) -> str:
    # ── PARSE ────────────────────────────────────────────────
    ...
    return generate_report(
        filepath=filepath, notes=notes, meta=meta,
        sections=sections, chords=chords,
        key_root=key_root, mode=mode, kconf=kconf,
        avg_bpm=avg_bpm, main_bpm=main_bpm,
        ts_str_val=ts_s, total_dur=total_dur,
        canon_progs=canon_progs, modal_borrow=modal_borrow,
        centricity=centricity, resolution=resolution,
        interval_anal=interval_anal, contour=contour, contour_p=cpitch,
        motifs=motifs, rhythm=rhythm, texture=texture, accomp=accomp,
        tempo_gestures=tempo_gestures, instruments=instruments,
        similar_secs=similar_secs, builds=builds,
        overall_tension=overall_tension,
        tension_curve=tension_curve, cadences=cadences,
        voice_leading=voice_leading, expectation=expectation,
        register_analysis=register_analysis,
        pedal_points=pedal_points, harmonic_graph=harmonic_graph,
        musical_form=musical_form, n_sections=n_sections,
        silence_analysis=silence_analysis, energy_profile=energy_profile,
        narrative_arc=narrative_arc, emotional_transitions=emotional_transitions,
        suspense=suspense, thematic_transforms=thematic_transforms,
        polarity=polarity, event_density=event_density,
        emotional_genre=emotional_genre, chromaticism=chromaticism,
        climax_comparison=climax_comparison, groove=groove, sva_list=sva_list,
        phrasing=phrasing, counterpoint=counterpoint,
        microexpression=microexpression, dissonance_res=dissonance_res,
        golden=golden, info_density=info_density,
        tonal_ambiguity=tonal_ambiguity, perceptual=perceptual,
        inner_voice=inner_voice, cinematic=cinematic,
        fingerprint=fingerprint, eternity=eternity,
        coherence=coherence, story=story,
        dyn_segments=dyn_segments, tonal_gravity=tonal_gravity,
        subtext=subtext, momentum=momentum,
        auditory_fatigue=auditory_fatigue, polyrhythm=polyrhythm,
        fractal=fractal, emotional_weight=emotional_weight,
        pnr=pnr, narrative_density=narrative_density,
        cumulative_state=cumulative_state,
        harmonic_color=harmonic_color, register_harmony=register_harmony,
        pitch_bend=pitch_bend,
        cultural=cultural, narrative_voice=narrative_voice,
        micro_timing=micro_timing, param_convergence=param_convergence,
        comfort_zones=comfort_zones, metric_hierarchy=metric_hierarchy,
        elliptical=elliptical, thematic_recurrence=thematic_recurrence,
        listener_model=listener_model, harmonic_semantic=harmonic_semantic,
        bass_narrative=bass_narrative, layer_sync=layer_sync,
...
def main():
    parser = argparse.ArgumentParser(
        description='Analizador musical y emocional avanzado v8.0  —  pip install mido'
    )
    parser.add_argument('filepath', help='Ruta al archivo .mid o .midi')
    parser.add_argument('--sections', type=int, default=6,
                        help='Número de secciones para el análisis (default: 6)')
    parser.add_argument('--output', type=str, default=None,
                        help='Guardar informe en un archivo .txt')
    parser.add_argument('--essay-only', action='store_true',
                        help='Generar solo el ensayo narrativo (sin métricas)')
    parser.add_argument('--essay-output', type=str, default=None,
                        help='Guardar el ensayo en un archivo separado')
    ...
    report = run_analysis(args.filepath, n_sections=args.sections)

    if getattr(args, 'essay_only', False):
        if '╔══ 71.' in report:
            start = report.index('╔══ 71.')
            end   = report.find('╔══ 58.', start)
            essay = report[start:end if end > 0 else len(report)]
            print('\n' + essay)
            if getattr(args, 'essay_output', None):
                with open(args.essay_output, 'w', encoding='utf-8') as f:
                    f.write(essay)
                print(f"\n✅  Ensayo guardado en: {args.essay_output}")
        else:
            print('\n' + report)
    else:
        print('\n' + report)
    ...
    if getattr(args, 'essay_output', None) and not getattr(args, 'essay_only', False):
        if '╔══ 71.' in report:
            start = report.index('╔══ 71.')
            end   = report.find('╔══ 58.', start)
            essay = report[start:end if end > 0 else len(report)]
            with open(args.essay_output, 'w', encoding='utf-8') as f:
                f.write(essay)
            print(f"✅  Ensayo guardado en: {args.essay_output}")
