...
find . -name '*.cpp' -o -name '*.hpp' -o -name '*.cc' -o -name '*.hh' -o -name '*.c' -o -name '*.h' | while read file
do
...
/usr/share/sonar-scanner-3.0.3.778-linux/bin/sonar-scanner \
  ...
  -D sonar.tests=$tests \
  -D sonar.exclusions=**/*.pb.h,**/*.pb.cc,**/*.capnp.c++,**/*.capnp.h,**/.release/**.*,**/.debug/**.*,CMakeLists.txt,**/vendor/** \
